...
def make_ort_session(model_path):
    ...
    if use_trt:
        trt_opts = {
            ...
            "trt_max_workspace_size": str(4 << 30),  # 4 GB
        }
        providers = [
    ...
    with open(str(model_path), "rb") as f:
        ...
        print(
            f"[ORT] requested={[p if isinstance(p, str) else p[0] for p in providers]}  actual={actual}"
        )
        return sess
...
def preload_csvs(csv_files):
    ...
    return dict(
        roll_lataccel=roll_la,
        v_ego=v_ego,
        a_ego=a_ego,
        target_lataccel=tgt_la,
        steer_command=steer,
        N=N,
        T=T,
    )
...
class CSVCache:
    ...
    def __init__(self, csv_files):
        ...
        seed_prefix = os.getenv("SEED_PREFIX", "data")  # prefix for seed path
        ...
        print(
            f"  [CSVCache] {N_all} files, T={self.T}, loaded in {_t.time() - t0:.1f}s",
            flush=True,
        )
...
class BatchedPhysicsModel:
    ...
    def __init__(self, model_path: str, ort_session=None) -> None:
        ...
        if self._use_gpu:
            ...
            self._bins_gpu = torch.from_numpy(
                self.tokenizer.bins.astype(np.float64)
            ).cuda()
    ...
    def predict(
        self, input_data: dict, temperature=0.8, rng_u=None, rngs=None
    ) -> np.ndarray:
        if self._use_gpu:
    ...
    def _ensure_gpu_bufs(self, N, CL):
        ...
        if N != self._cached_N:
            torch = self._torch
            self._out_gpu = torch.empty(
                (N, CL, VOCAB_SIZE), dtype=torch.float32, device="cuda"
            )
            self._states_gpu = torch.empty(
                (N, CL, 4), dtype=torch.float32, device="cuda"
            )
            self._tokens_gpu = torch.empty((N, CL), dtype=torch.int64, device="cuda")
            self._clamped_buf = torch.empty((N, CL), dtype=torch.float64, device="cuda")
    ...
    def _predict_gpu(self, input_data, temperature, rng_u):
        ...
        io.bind_input(
            "states", "cuda", 0, np.float32, self._states_shape, states_gpu.data_ptr()
        )
        io.bind_input(
            "tokens", "cuda", 0, np.int64, self._tokens_shape, tokens_gpu.data_ptr()
        )
        io.bind_output(
            self._out_name,
            "cuda",
        ...
        samples = (
            torch.searchsorted(cdf.double(), u.double(), right=True)
            .squeeze(1)
    ...
    def _predict_cpu(self, input_data, temperature, rng_u, rngs):
        ...
        if rng_u is not None:
            ...
            u = np.random.rand(N)
        samples = np.array(
            [np.searchsorted(cdf[i], u[i], side="right") for i in range(N)],
            dtype=np.intp,
        )
    ...
    def get_current_lataccel(
        ...
        _temp = getattr(self, "sim_temperature", 0.8)
        sampled = self.tokenizer.decode(
            self._predict_cpu(input_data, temperature=_temp, rng_u=rng_u, rngs=rngs)
        )
    ...
    def _get_current_lataccel_gpu(
        self, sim_states, actions, past_preds, rng_u, return_expected
    ):
        ...
        _temp = getattr(self, "sim_temperature", 0.8)
        sample_tokens = self.predict(input_data, temperature=_temp, rng_u=rng_u)
...
class BatchedSimulator:
    ...
    def reset(self) -> None:
        ...
        if self._gpu:
            _torch = self._torch
            self.action_history = _torch.zeros(
                (N, T), dtype=_torch.float64, device="cuda"
            )
            self.action_history[:, :CL] = self.data_gpu["steer_command"][:, :CL]
            self.state_history = _torch.zeros(
                (N, T, 3), dtype=_torch.float64, device="cuda"
            )
            ...
            self.current_lataccel_history = _torch.zeros(
                (N, T), dtype=_torch.float64, device="cuda"
            )
            self.current_lataccel_history[:, :CL] = self.data_gpu["target_lataccel"][
                :, :CL
            ]
    ...
    # ── snapshot / restore for forked rollouts ─────────────────

    def snapshot(self):
        """Capture minimal state needed to fork the simulator.
        Returns an opaque dict that can be passed to restore().
        Only captures the mutable autoregressive state, not CSV data."""
        if self._gpu:
            return {
                "action_history": self.action_history.clone(),
                "state_history": self.state_history.clone(),
                "current_lataccel_history": self.current_lataccel_history.clone(),
                "current_lataccel": self.current_lataccel.clone(),
                "_hist_len": self._hist_len,
            }
        return {
            "action_history": self.action_history.copy(),
            "state_history": self.state_history.copy(),
            "current_lataccel_history": self.current_lataccel_history.copy(),
    ...
    def restore(self, snap):
        ...
        The sim must have the same N and T as when the snapshot was taken."""
        if self._gpu:
            self.action_history.copy_(snap["action_history"])
            self.state_history.copy_(snap["state_history"])
            self.current_lataccel_history.copy_(snap["current_lataccel_history"])
            self.current_lataccel.copy_(snap["current_lataccel"])
        else:
            self.action_history[:] = snap["action_history"]
            self.state_history[:] = snap["state_history"]
            self.current_lataccel_history[:] = snap["current_lataccel_history"]
            self.current_lataccel[:] = snap["current_lataccel"]
        self._hist_len = snap["_hist_len"]

    def rollout_from(self, start_step, end_step, controller_fn):
        """Run rollout from start_step to end_step (exclusive).
        Assumes the simulator state has been set (via restore) to the state
        at start_step. Returns cost dict computed over the standard cost window."""
        if self._gpu:
            _torch = self._torch
            dg = self.data_gpu
            ...
            for step_idx in range(start_step, end_step):
                ...
                actions = controller_fn(
                    step_idx, target, cur_la_np, state_dict, future_plan
                )
                h = self._hist_len
    ...
    def sim_step(self, step_idx: int) -> None:
        ...
        if self._gpu:
            ...
            if step_idx >= CONTROL_START_IDX:
                ...
                self.current_lataccel = self.data_gpu["target_lataccel"][
                    :, step_idx
                ].clone()
    ...
    def step(self, step_idx: int, actions) -> dict:
        ...
        Mirrors:
          ...
          sim_step(step_idx)
        """
        roll_la, v_ego, a_ego, target, future_plan = self.get_state_target_futureplan(
            step_idx
        )
    ...
    def rollout(self, controller_fn: Callable) -> Dict[str, np.ndarray]:
        ...
        if int(os.environ.get("DEBUG", "0")) >= 2:
            print(
                f"  [rollout N={self.N}] ctrl={t_ctrl:.1f}s  sim={t_sim:.1f}s  total={t_ctrl + t_sim:.1f}s",
                flush=True,
            )
        return self.compute_cost()
    ...
    def compute_cost(self) -> Dict[str, np.ndarray]:
        ...
        if self._gpu:
            _torch = self._torch
            target_gpu = self.data_gpu["target_lataccel"][
