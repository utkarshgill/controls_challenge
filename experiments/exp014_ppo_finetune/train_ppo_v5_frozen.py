"""
Experiment 014: PPO Fine-Tuning from BC (V5 - FROZEN TRUNK)

RADICAL CHANGE:
1. **FREEZE the trunk** (feature extractor) - DON'T touch it!
2. Only train actor_head (final layer) and critic_head
3. Much higher LR is OK now (1e-3) since we're only training output layer
4. This prevents breaking the good feature representation

V3/V4 Problem: Training breaks the trunk → policy collapses
V5 Solution: Keep trunk frozen → only fine-tune outputs

This is likely how the winner did it!
"""
import sys
...
NUM_EPOCHS_PPO = 4              # Can do more epochs since only training output

LEARNING_RATE = 1e-3            # Can be higher since only training output layers
GAMMA = 0.99                    # Discount factor
GAE_LAMBDA = 0.95               # GAE parameter
CLIP_EPSILON = 0.2              # Standard clip is fine now
...
def main():
    print("="*80)
    print("Experiment 014: PPO Fine-Tuning (V5 - FROZEN TRUNK)")
    print("="*80)
    ...
    # FREEZE THE TRUNK - don't touch the feature extractor!
    for param in actor_critic.trunk.parameters():
        param.requires_grad = False
    
    trainable_params = sum(p.numel() for p in actor_critic.parameters() if p.requires_grad)
    frozen_params = sum(p.numel() for p in actor_critic.parameters() if not p.requires_grad)
    
    print(f"\n✓ Actor-Critic created:")
    print(f"  State dim: {STATE_DIM}")
    print(f"  Total parameters: {sum(p.numel() for p in actor_critic.parameters()):,}")
    print(f"  🔒 Frozen (trunk): {frozen_params:,}")
    print(f"  🔓 Trainable (heads): {trainable_params:,}")
    ...
    for iteration in trange(NUM_ITERATIONS, desc="Training"):
        ...
        if mean_cost < best_cost:
            ...
            print(f"  💾 Saved new best: {mean_cost:.2f}")
