...
**Key insight**: PID is a local optimum. PPO should not learn full control,
but only small anticipatory corrections in the feedforward nullspace.

Architecture: u = u_PID(s) + ε × π_θ(s_preview)

Where:
- u_PID: Frozen PID baseline (provides feedback stability)
- π_θ: Learned residual (small anticipatory adjustments)
- ε: Scaling factor (0.1 initially - residual is 10% of PID magnitude)
- s_preview: State emphasizing future curvatures

Physics rationale:
- PID handles reactive control (error correction)
- Residual handles predictive control (curve anticipation)
- Small ε ensures we stay in PID's stability basin
- Low-pass filtering prevents high-frequency oscillations (jerk penalty)
"""
import sys
...
ACTION_DIM = 1  # [residual correction]
...
HIDDEN_DIM = 128
ACTOR_LAYERS = 3
CRITIC_LAYERS = 2  # Shallow - critic is blind to hidden state (PID state, simulator history)
PI_LR = 3e-4
VF_LR = 1e-3
GAMMA = 0.99
GAE_LAMBDA = 0.95
EPS_CLIP = 0.2
VF_COEF = 0.05  # Heavily reduced - this is a POMDP, critic hallucinates otherwise
ENTROPY_COEF = 0.01

# Key fixes for residual PPO in POMDP with delayed effects:
# 1. VF_COEF=0.05: Nerf critic (it doesn't see PID state, simulator history, filter state)
# 2. Reward shaping: Bonus for error reduction (credit assignment for anticipation)
# 3. Residual warmup: Force PID to settle before residual activates (manifold alignment)
# 4. Low exploration: σ≈0.3 not 1.0 (smoothness constraint)
...
class TinyPhysicsGymEnv:
    ...
    def _get_state(self):
        """Construct 55D state vector with proper normalization"""
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, actor_layers, critic_layers):
        ...
        # Initialize with lower noise for residual learning (σ≈0.3 instead of 1.0)
...
def train():
    print("="*80)
    print("Experiment 032: Residual PPO (Nullspace Learning)")
    print("="*80)
    print(f"Architecture: u = u_PID + ε × π_θ(s)")
