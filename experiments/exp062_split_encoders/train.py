# exp062 — Split encoders: separate past/future/core encoders
#
# Fork of exp055 (gold standard, world #3).
# Only change: ActorCritic uses separate encoders for core(16), past(40),
# future(200) with proportional capacity + noise on past during training.
# Everything else identical: obs layout, rollout, BC, PPO, train loop.
...
PAST_NOISE = float(
    os.getenv("PAST_NOISE", "0.012")
)  # gaussian σ on past features (train only)
...
# Obs slices for split encoders
CORE_DIM = C  # 16
PAST_DIM = H2 - C  # 40  (20 h_act + 20 h_lat)
FUT_DIM = OBS_DIM - H2  # 200 (50×4 future channels)
HEAD_IN = 384  # 64 + 64 + 256 = concat of encoder outputs


def _make_encoder(in_dim, hidden, out_dim, n_layers):
...
class SplitNet(nn.Module):
    """Separate encoders for core(16) / past(40) / future(200), shared head.

    Used as both actor and critic — drop-in replacement for nn.Sequential.
    Supports ac.actor(obs), ac.actor.parameters(), ac.actor.train(), etc.

    Noise injection on past features: only active when self.training=True
    and self.past_noise > 0.
    """

    def __init__(self, out_dim, past_noise=0.0):
        super().__init__()
        self.core_enc = _make_encoder(CORE_DIM, 64, 64, 1)  # 16 → 64   (~1K)
        self.past_enc = _make_encoder(PAST_DIM, 128, 64, 2)  # 40 → 128 → 64  (~10K)
        self.future_enc = _make_encoder(FUT_DIM, 384, 256, 2)  # 200 → 384 → 256 (~150K)
        ...
        for layer in self.head[:-1]:
            _ortho(layer)
        self.past_noise = past_noise

    def forward(self, obs):
        core = obs[:, :C]
        past = obs[:, C:H2]
        fut = obs[:, H2:]
        if self.past_noise > 0.0 and self.training:
            past = past + torch.randn_like(past) * self.past_noise
        h = torch.cat(
            [
                self.core_enc(core),
                self.past_enc(past),
...
class ActorCritic(nn.Module):
    def __init__(self):
        super().__init__()
        self.actor = SplitNet(out_dim=2, past_noise=PAST_NOISE)
        self.critic = SplitNet(out_dim=1, past_noise=0.0)
        _ortho(self.actor.head[-1], gain=0.01)
        _ortho(self.critic.head[-1], gain=1.0)
