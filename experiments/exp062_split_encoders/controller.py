"""exp062 — Split encoder controller (test time, no noise).

Identical to exp055 controller except ActorCritic uses SplitNet
with separate core/past/future encoders. No noise at test time.
"""
...
C = 16
H2 = C + HIST_LEN + HIST_LEN  # 56
OBS_DIM = H2 + FUTURE_K * 4  # 256
CORE_DIM = C  # 16
PAST_DIM = H2 - C  # 40
FUT_DIM = OBS_DIM - H2  # 200
HEAD_IN = 384
...
class SplitNet(nn.Module):
    def __init__(self, out_dim, past_noise=0.0):
        super().__init__()
        self.core_enc = _make_encoder(CORE_DIM, 64, 64, 1)
        self.past_enc = _make_encoder(PAST_DIM, 128, 64, 2)
        self.future_enc = _make_encoder(FUT_DIM, 384, 256, 2)
    ...
    def forward(self, obs):
        ...
        past = obs[:, C:H2]
        fut = obs[:, H2:]
        # No noise at test time (self.training=False after .eval())
        if self.past_noise > 0.0 and self.training:
            past = past + torch.randn_like(past) * self.past_noise
        h = torch.cat(
            [self.core_enc(core), self.past_enc(past), self.future_enc(fut)], dim=-1
        )
        return self.head(h)
...
class ActorCritic(nn.Module):
    def __init__(self):
        super().__init__()
        self.actor = SplitNet(out_dim=2)
        self.critic = SplitNet(out_dim=1)
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp062_split_encoders"
        ckpt = None
