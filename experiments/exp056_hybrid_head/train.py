#!/usr/bin/env python3
# exp056 — exp055 with a concat-split actor
#
...
class ConcatSplitActor(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.branch_slices = (
            slice(0, self.core_hidden),
            slice(self.core_hidden, self.core_hidden + self.hist_hidden),
            slice(self.core_hidden + self.hist_hidden, HIDDEN),
        )
        branch_specs = (
            ('core_layers', C, self.core_hidden, self.branch_slices[0]),
            ('hist_layers', hist_dim, self.hist_hidden, self.branch_slices[1]),
            ('future_layers', future_dim, self.future_hidden, self.branch_slices[2]),
        )
        for name, in_dim, branch_hidden, _ in branch_specs:
            layers = nn.ModuleList()
            layers.append(nn.Linear(in_dim, branch_hidden))
            for _ in range(1, self.concat_layer):
                layers.append(nn.Linear(branch_hidden, branch_hidden))
            setattr(self, name, layers)

        shared = []
        for _ in range(self.concat_layer, A_LAYERS):
            shared += [nn.Linear(HIDDEN, HIDDEN), nn.ReLU()]
        self.trunk = nn.Sequential(*shared)
        self.out = nn.Linear(HIDDEN, 2)

        full_hidden = [nn.Linear(STATE_DIM, HIDDEN)]
        full_hidden += [nn.Linear(HIDDEN, HIDDEN) for _ in range(1, A_LAYERS)]
        for layer in full_hidden:
            _ortho(layer)
        full_out = nn.Linear(HIDDEN, 2)
        _ortho(full_out, gain=0.01)

        with torch.no_grad():
            input_slices = (slice(0, C), slice(C, H2), slice(H2, OBS_DIM))
            branch_names = ('core_layers', 'hist_layers', 'future_layers')
            for branch_name, input_slice, row_slice in zip(branch_names, input_slices, self.branch_slices):
                branch_layers = getattr(self, branch_name)
                branch_layers[0].weight.copy_(full_hidden[0].weight[row_slice, input_slice])
                branch_layers[0].bias.copy_(full_hidden[0].bias[row_slice])
                for depth in range(1, self.concat_layer):
                    branch_layers[depth].weight.copy_(full_hidden[depth].weight[row_slice, row_slice])
                    branch_layers[depth].bias.copy_(full_hidden[depth].bias[row_slice])

            for depth in range(self.concat_layer, A_LAYERS):
                layer = self.trunk[2 * (depth - self.concat_layer)]
                layer.weight.copy_(full_hidden[depth].weight)
                layer.bias.copy_(full_hidden[depth].bias)

            self.out.weight.copy_(full_out.weight)
            self.out.bias.copy_(full_out.bias)
...
def train():
    actor_cfg = actor_cfg_from_env()
    ac = ActorCritic(actor_cfg).to(DEV)
    ...
    if RESUME and BEST_PT.exists():
        ...
        ckpt_actor_cfg = ckpt.get('actor_cfg', actor_cfg)
        if ckpt_actor_cfg != actor_cfg:
            raise ValueError(f"Checkpoint actor_cfg {ckpt_actor_cfg} does not match current actor_cfg {actor_cfg}")
    ...
    print(f"  actor=concat_split  concat_layer={actor_cfg['concat_layer']}  dims={actor_cfg['core_hidden']}/{actor_cfg['hist_hidden']}/{actor_cfg['future_hidden']}")
    ...
    torch.save({'ac': ac.state_dict(), 'actor_cfg': actor_cfg, 'delta_scale': cur_ds}, EXP_DIR / 'final_model.pt')
