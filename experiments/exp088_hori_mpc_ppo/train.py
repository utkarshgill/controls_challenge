# exp088 — PPO + MPPI dual-buffer with adaptive mixing (Hori et al.)
#
# Two data sources, one PPO loss:
#   D_RL:   real stochastic sim rollouts (standard PPO collection)
#   D_MPPI: expected-value sim rollouts (MPPI candidates from policy)
#
# PPO loss = (1-ρ) * L(D_RL) + ρ * L(D_MPPI)
# ρ adapts based on critic ensemble uncertainty: high uncertainty → more MPPI data
#
# Both rollouts use existing TRT batched sim. MPPI rollout just sets use_expected=True
# and tiles M copies per route. All using same infrastructure.
...
# ── MPPI dual-buffer (Hori et al.) ────────────────────────────
MPPI_M = int(os.getenv("MPPI_M", "10"))  # copies per route for MPPI rollout
MPPI_CSVS = int(os.getenv("MPPI_CSVS", "5000"))  # total MPPI rollouts (routes * M)
RHO_INIT = float(os.getenv("RHO_INIT", "0.3"))  # initial mixing ratio
RHO_LAMBDA = float(os.getenv("RHO_LAMBDA", "0.98"))  # EMA decay for adaptive ρ
N_CRITICS = int(os.getenv("N_CRITICS", "3"))  # critic ensemble for uncertainty
...
class PPO:
    ...
    def update_dual(self, gd_rl, gd_mppi, rho, critic_only=False, ds=DELTA_SCALE_MAX):
        """Dual-buffer PPO update. Concatenates D_RL and D_MPPI with weighted loss."""

        # Prepare both buffers
        def prep(gd, k_samples):
            obs = gd["obs"]
            raw = gd["raw"].unsqueeze(-1)
            adv_t, ret_t = self._gae(gd["rew"], gd["val_2d"], gd["done"])
            if k_samples > 1:
                n_total, S = gd["rew"].shape
                n_routes = n_total // k_samples
                adv_2d = adv_t.reshape(n_routes, k_samples, -1)
            ...
            return obs, x_t, adv_t, ret_t, old_lp

        obs_rl, x_rl, adv_rl, ret_rl, olp_rl = prep(gd_rl, SAMPLES_PER_ROUTE)
        obs_mp, x_mp, adv_mp, ret_mp, olp_mp = prep(gd_mppi, MPPI_M)

        # Concatenate with weights
        obs = torch.cat([obs_rl, obs_mp])
        x_t = torch.cat([x_rl, x_mp])
        adv_t = torch.cat([adv_rl, adv_mp])
        ret_t = torch.cat([ret_rl, ret_mp])
        old_lp = torch.cat([olp_rl, olp_mp])
        n_rl = obs_rl.shape[0]
        n_mp = obs_mp.shape[0]
        weights = torch.cat(
            [
                torch.full((n_rl,), 1.0 - rho, device="cuda"),
                torch.full((n_mp,), rho, device="cuda"),
            ]
        )
        # Normalize weights so they average to 1
        weights = weights * len(weights) / weights.sum()

        # Now run standard PPO update with weighted loss
        return self._update_impl(
            obs, x_t, adv_t, ret_t, old_lp, weights, critic_only, ds
        )

    def _update_impl(self, obs, x_t, adv_t, ret_t, old_lp, weights, critic_only, ds):
        """Weighted PPO update implementation."""
        ds = float(ds)
    ...
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        """Standard single-buffer PPO update (for non-MPPI epochs)."""
        obs = gd["obs"]
        raw = gd["raw"].unsqueeze(-1)
        adv_t, ret_t = self._gae(gd["rew"], gd["val_2d"], gd["done"])
        ...
        weights = torch.ones(len(obs), device="cuda")
        return self._update_impl(
            obs, x_t, adv_t, ret_t, old_lp, weights, critic_only, ds
        )
...
def train():
    ...
    print(
        ...
        f"\n  MPPI: M={MPPI_M} csvs={MPPI_CSVS} ρ_init={RHO_INIT} λ={RHO_LAMBDA}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        for pg in ppo.vf_opt.param_groups:
            pg["lr"] = vf_lr
        t0 = time.time()
        # D_RL: real stochastic rollout
        n_routes_rl = min(CSVS_EPOCH, len(tr_f)) // SAMPLES_PER_ROUTE
        batch_rl = random.sample(tr_f, max(n_routes_rl, 1))
        batch_rl = [f for f in batch_rl for _ in range(SAMPLES_PER_ROUTE)]
        res_rl = rollout(
            batch_rl, ac, mdl_path, ort_sess, csv_cache, deterministic=False, ds=ds
        )

        # D_MPPI: expected-value rollout (MPPI virtual data)
        n_routes_mp = min(MPPI_CSVS, len(tr_f)) // MPPI_M
        batch_mp = random.sample(tr_f, max(n_routes_mp, 1))
        batch_mp = [f for f in batch_mp for _ in range(MPPI_M)]
        res_mppi = rollout(
            batch_mp,
            ac,
            mdl_path,
        ...
        # Adaptive ρ from critic ensemble uncertainty (simplified: use value variance across routes)
        if not co and epoch > 0:
            with torch.no_grad():
                vals = ac.critic(res_rl["obs"][:5000]).squeeze(-1)
                sigma_v = vals.std().item()
                omega = 1.0 / (1.0 + sigma_v**2)
                rho_omega_ema = (
                    RHO_LAMBDA * getattr(ppo, "_omega_ema", omega)
                    + (1.0 - RHO_LAMBDA) * omega
                )
                ppo._omega_ema = rho_omega_ema
                eta = (1.0 - RHO_LAMBDA) * rho_omega_ema
                cur_rho = max(0.0, getattr(ppo, "_rho", RHO_INIT) * (1.0 - eta))
                ppo._rho = cur_rho
        else:
            cur_rho = RHO_INIT

        if co:
            info = ppo.update(res_rl, critic_only=True, ds=ds)
        else:
            info = ppo.update_dual(res_rl, res_mppi, cur_rho, critic_only=False, ds=ds)
        tu = time.time() - t1

        phase = "  [critic warmup]" if co else ""
        line = (
            f"E{epoch:3d}  rl={np.mean(res_rl['costs']):5.1f}  mppi={np.mean(res_mppi['costs']):5.1f}"
            f"  ρ={cur_rho:.3f}  σ={info['σ']:.4f}  σraw={info['σraw']:.4f}"
