# exp085 — REINFORCE with cost-weighted best-of-K, forced high sigma
#
# Same rollout as exp055 (K copies per route, batched GPU sim).
# Replace PPO with REINFORCE: no importance ratios, no critic.
# Force high sigma so K copies are actually different.
# Weight each trajectory's log-prob by -(cost - route_mean_cost).
# The best trajectory per route gets pushed toward, worst pushed away.
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        if deterministic:
            ...
            # Sample from temperature-widened distribution for diversity
            a_t = (a_p / SAMPLE_TEMP).clamp_min(0.01)
            b_t = (b_p / SAMPLE_TEMP).clamp_min(0.01)
            x = torch.distributions.Beta(a_t, b_t).sample()
            raw_policy = 2.0 * x - 1.0
            # Log-prob under the ORIGINAL policy (for REINFORCE gradient)
            x_clamped = x.clamp(1e-6, 1 - 1e-6)
            logp = torch.distributions.Beta(a_p, b_p).log_prob(x_clamped)
...
class PPO:
    ...
    def update(self, gd, ds=DELTA_SCALE_MAX):
        """REINFORCE with per-route cost centering. No importance ratios."""
        obs = gd["obs"]  # (N*S,  OBS_DIM) — all trajectories flattened
        raw = gd["raw"]  # (N*S,)
        costs = gd["costs"]  # (N_total,) numpy — per-trajectory total cost

        N_total = len(costs)
        S = len(obs) // N_total
        ds = float(ds)

        # Per-route cost centering: weight = -(cost - route_mean_cost)
        # Trajectories are ordered: [r0c0, r0c1, ..., r0cK, r1c0, ...]
        K = SAMPLES_PER_ROUTE
        n_routes = N_total // K
        costs_t = torch.from_numpy(costs).to("cuda").float().view(n_routes, K)
        centered = -(costs_t - costs_t.mean(dim=1, keepdim=True))  # (n_routes, K)
        # Normalize per route
        std = centered.std(dim=1, keepdim=True).clamp(min=1e-8)
        centered = centered / std
        # Expand to per-step weights: (N_total, S) -> (N_total * S,)
        per_traj_weight = centered.view(-1)  # (N_total,)
        per_step_weight = (
            per_traj_weight.unsqueeze(1).expand(-1, S).reshape(-1)
        )  # (N_total*S,)
        ...
        # Single pass (REINFORCE — no reuse like PPO)
        for idx in torch.randperm(len(obs), device="cuda").split(MINI_BS):
            ...
            lp = dist.log_prob(x_t[idx])
            # REINFORCE loss: -E[weight * log_prob]
            pi_loss = -(per_step_weight[idx] * lp).mean()
        ...
        return dict(
            pi=(pi_sum / max(1, n_actor)),
            vf=0.0,
            ent=(ent_sum / max(1, n_actor)),
            σ=sigma_eff,
            σraw=sigma_raw,
            σpen=0.0,
...
def train():
    ...
    print(
        ...
        f"  Δscale={'decay' if DELTA_SCALE_DECAY else 'fixed'} {ds_max_run}→{ds_min_run}  dim={STATE_DIM}"
        f"\n  REINFORCE: sample_temp={SAMPLE_TEMP}  K={SAMPLES_PER_ROUTE}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        t1 = time.time()
        info = ppo.update(res, ds=ds)
        tu = time.time() - t1

        line = (
            f"E{epoch:3d}  train={np.mean(res['costs']):6.1f}  σ={info['σ']:.4f}  σraw={info['σraw']:.4f}"
            f"  π={info['pi']:+.4f}  H={info['ent']:.2f}"
            f"  Δs={ds:.4f}  lr={info['lr']:.1e}  ⏱{t1 - t0:.0f}+{tu:.0f}s"
        )
