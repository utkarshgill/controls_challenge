# exp063 — Finite-difference policy gradient (surgeon's eyes)
#
# Fork of exp055 (gold standard, world #3).
# Change: during rollout, compute ∂E[lataccel]/∂action via finite differences
# and store the optimal delta correction per step. PPO update includes an
# auxiliary loss that pushes the policy toward the FD-optimal action.
# This gives per-step, action-resolution gradient signal — not episode-level.
...
FD_EPS = float(os.getenv("FD_EPS", "0.005"))  # steer perturbation for ∂lataccel/∂action
FD_COEF = float(
    os.getenv("FD_COEF", "0.1")
)  # weight of FD auxiliary loss in PPO update
FD_LR_MULT = float(
    os.getenv("FD_LR_MULT", "0.1")
)  # step size for FD-optimal correction
...
def rollout(
    ...
    # ── Finite-difference gradient: ∂E[lataccel]/∂action per step ─────────
    # For each step t, probe the physics model at action ± ε to estimate
    # the Jacobian, then compute the optimal raw correction.
    # This gives per-step, action-resolution gradient signal.
    fd_optimal_raw = torch.zeros((N, S), dtype=torch.float32, device="cuda")
    if FD_COEF > 0.0:
        sim.compute_expected = True  # ensure E[lataccel] is available
        raw_policy_2d = all_raw[:S].T  # (N, S) — the raw deltas the policy chose
        for t_local in range(S):
            step_idx = start + t_local
            h = CL + (step_idx - CL)  # = step_idx, the hist index
            # Context slices for the physics model
            ctx_states = sim.state_history[:, h - CL + 1 : h + 1, :]
            ctx_preds = sim.current_lataccel_history[:, h - CL : h]
            ctx_acts = sim.action_history[:, h - CL + 1 : h + 1].clone()
            orig_act = ctx_acts[:, -1].clone()

            # Probe at action + ε
            ctx_acts[:, -1] = (orig_act + FD_EPS).clamp(STEER_RANGE[0], STEER_RANGE[1])
            _, e_plus = sim.sim_model.get_current_lataccel(
                ctx_states,
                ctx_acts,
                ctx_preds,
                rng_u=sim._rng_all_gpu[step_idx - CL],
                return_expected=True,
            )
            # Probe at action - ε
            ctx_acts[:, -1] = (orig_act - FD_EPS).clamp(STEER_RANGE[0], STEER_RANGE[1])
            _, e_minus = sim.sim_model.get_current_lataccel(
                ctx_states,
                ctx_acts,
                ctx_preds,
                rng_u=sim._rng_all_gpu[step_idx - CL],
            ...
            # ∂E/∂a ≈ (E+ - E-) / (2ε)
            dE_da = (e_plus.float() - e_minus.float()) / (2.0 * FD_EPS)
            # E[lataccel] at the actual action (use pred from sim history)
            e_nominal = pred[:, t_local]
            tgt = target[:, t_local]
            error = tgt - e_nominal  # how far off are we
            # Newton step: correction to action that would zero the error
            # Δaction = error / (∂E/∂a)  (one-step linearisation)
            safe_dE = dE_da.clamp(min=0.01)  # avoid division by tiny/negative Jacobian
            safe_dE = torch.where(dE_da.abs() > 0.01, dE_da, safe_dE)
            optimal_delta_correction = error / safe_dE  # steer units
            # Convert to raw space and add to the policy's raw output
            raw_correction = (optimal_delta_correction / ds).clamp(
                -1.0, 1.0
            ) * FD_LR_MULT
            fd_optimal_raw[:, t_local] = (
                raw_policy_2d[:, t_local] + raw_correction
            ).clamp(-1.0, 1.0)
    ...
    return dict(
        ...
        costs=costs,
        fd_raw=fd_optimal_raw.reshape(-1) if FD_COEF > 0.0 else None,
    )
...
class PPO:
    ...
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        ...
        # FD optimal targets (if available)
        fd_raw = gd.get("fd_raw", None)
        if fd_raw is not None:
            fd_x_t = ((fd_raw.unsqueeze(-1) + 1) / 2).clamp(1e-6, 1 - 1e-6)
        ...
        for _ in range(K_EPOCHS):
            for idx in torch.randperm(len(obs), device="cuda").split(MINI_BS):
                ...
                if critic_only:
                    ...
                    # FD auxiliary loss: push policy toward FD-optimal action
                    fd_loss = torch.tensor(0.0, device="cuda")
                    if fd_raw is not None and FD_COEF > 0.0:
                        fd_nll = -dist.log_prob(fd_x_t[idx].squeeze(-1)).mean()
                        fd_loss = fd_nll
                        fd_sum += fd_loss.detach().item() * bs
                    loss = (
                        pi_loss
                        + VF_COEF * vf_loss
                        - ENT_COEF * ent
        ...
        return dict(
            ...
            fd=(fd_sum / max(1, n_actor)) if not critic_only else 0.0,
...
def train():
    ...
    for epoch in range(MAX_EP):
        ...
        line = (
            ...
            f"  σpen={info['σpen']:.4f}  π={info['pi']:+.4f}  fd={info['fd']:.3f}  vf={info['vf']:.1f}  H={info['ent']:.2f}"
