"""
exp021: PPO - FIXED advantage computation per episode
"""
import sys
...
class ActorCritic(nn.Module):
    ...
    def act(self, raw_state, det=False, internals=False):
        s = torch.as_tensor(raw_state / OBS_SCALE, dtype=torch.float32).to(device)
        with torch.no_grad():
            mean, std, _ = self(s)
            a = mean if det else torch.distributions.Normal(mean, std).sample()
        return (a.cpu().numpy(), s, a) if internals else a.cpu().numpy()
...
class Ctrl:
    ...
    def update(self, target, current, state, future_plan):
        ...
        if self.collect:
            a_np, s_t, a = self.ac.act(raw_state, det=False, internals=True)
            ...
            self.pl = current
            self.sc += 1
            return float(np.clip(a_np.item(), -2.0, 2.0))
        else:
            return float(np.clip(self.ac.act(raw_state, det=True).item(), -2.0, 2.0))

def compute_advantages_per_episode(rewards, values, gamma, lam):
    """GAE for single episode"""
    T = len(rewards)
    advs, gae = [], 0.0
    ...
    def update(self, ctrls):
        ...
        for ctrl in ctrls:
            ...
            with torch.no_grad():
                ...
                # Compute advantages for THIS episode only
        ...
        # Concatenate after per-episode computation
...
def train():
    ...
    print(f"BC init: {evaluate(ac, test_files):.2f}, σ={ac.log_std.exp().item():.4f}")
    
    ppo = PPO(ac)
    best = evaluate(ac, test_files)
    ...
    for ep in trange(max_epochs):
        ...
        total_loss, actor_loss, critic_loss = ppo.update(ctrls)
        
        if ep % eval_interval == 0:
            val_cost = evaluate(ac, val_files, 10)
            ...
            print(f"E{ep:2d} tr={np.mean(costs):5.1f} val={val_cost:5.1f} tst={test_cost:5.1f} best={best:5.1f} σ={ac.log_std.exp().item():.4f}", flush=True)
