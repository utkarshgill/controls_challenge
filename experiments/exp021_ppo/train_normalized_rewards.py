"""PPO with reward normalization to help critic learn"""
import sys
...
max_epochs, routes_per_epoch, eval_interval = 100, 30, 5
...
def compute_advantages_per_episode(rewards_norm, values, gamma, lam):
    T = len(rewards_norm)
    advs, gae = [], 0.0
    vals_list = values.squeeze().tolist() + [0.0]
    for t in reversed(range(T)):
        delta = rewards_norm[t] + gamma * vals_list[t+1] - vals_list[t]
...
class PPO:
    ...
    def update(self, ctrls):
        # Collect all rewards first for normalization
        all_rewards = []
        for ctrl in ctrls:
        ...
        for ctrl in ctrls:
            ...
            # Normalize rewards
            rewards_norm = reward_normalizer.normalize(np.array(ctrl.rewards))
            ...
            with torch.no_grad():
                ...
                advs, rets = compute_advantages_per_episode(rewards_norm, values, gamma, gae_lambda)
...
def train():
    ...
    for ep in trange(max_epochs):
        ...
        if ep % eval_interval == 0:
            ...
            print(f"E{ep:2d} tr={np.mean(costs):5.1f} tst={test_cost:5.1f} best={best:5.1f} σ={ac.log_std.exp().item():.4f} r_norm=({reward_normalizer.mean:.2f}±{np.sqrt(reward_normalizer.var):.2f})", flush=True)
    
    print(f"\n✅ Best: {best:.2f} (BC was {bc_cost:.2f}, improvement={bc_cost-best:.2f})")
