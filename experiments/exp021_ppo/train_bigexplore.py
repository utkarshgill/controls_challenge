"""PPO with MUCH larger exploration - σ=0.05 instead of 0.007"""
import sys
...
max_epochs, routes_per_epoch, eval_interval = 50, 20, 2
...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.log_std = nn.Parameter(torch.zeros(action_dim) - 3.0)  # σ ≈ 0.05
    ...
    def update(self, target, current, state, future_plan):
        ...
        if self.collect:
            ...
            if self.pl and self.sc >= CONTROL_START_IDX and self.sc < COST_END_IDX:
                ...
                r = -(lat_err * LAT_ACCEL_COST_MULTIPLIER + jerk) / 100.0  # Larger rewards
...
def train():
    ...
    print(f"BC init: {bc_cost:.2f}, σ={ac.log_std.exp().item():.4f} (BIG exploration)\n")
        ...
        if ep % eval_interval == 0:
            ...
            if test_cost < best:
                ...
                torch.save({'model_state_dict': ac.state_dict(), 'obs_scale': OBS_SCALE}, 
                           'experiments/exp021_ppo/model_bigexplore.pth')
            print(f"E{ep:2d} tr={np.mean(costs):6.1f} tst={test_cost:5.1f} best={best:5.1f} σ={ac.log_std.exp().item():.4f}", flush=True)
    
    print(f"\n✅ Best: {best:.2f} (BC was {bc_cost:.2f}, Δ={bc_cost-best:.2f})")
