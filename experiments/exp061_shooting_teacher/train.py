#!/usr/bin/env python3
# exp061 — exp059 (Gaussian PPO) + GPU train-time shooting teacher
#
# Inherits everything from exp059 (squashed Gaussian, 258-dim obs, delta actions).
# Adds train-time shooting teacher: sample K first-actions from the current policy,
# roll them forward under deterministic policy continuation + expected plant dynamics,
# then distill the best first action back into the actor via weighted Gaussian NLL.
...
base = importlib.import_module("experiments.exp059_precision_tracking.train")
...
LOG_SIGMA_MIN = base.LOG_SIGMA_MIN
LOG_SIGMA_MAX = base.LOG_SIGMA_MAX
ACT_SMOOTH = base.ACT_SMOOTH
REWARD_SCALE = base.REWARD_SCALE
MINI_BS = base.MINI_BS
CRITIC_WARMUP = base.CRITIC_WARMUP
ELITE_BC_COEF = base.ELITE_BC_COEF
ELITE_BC_FRAC = base.ELITE_BC_FRAC
...
MAX_CONTROL_STEPS = base.MAX_CONTROL_STEPS
...
_squashed_gaussian_sample = base._squashed_gaussian_sample
_squashed_gaussian_logprob = base._squashed_gaussian_logprob
...
def _build_obs_teacher(
    ...
    """Build obs for teacher rollouts (handles temporal features)."""
...
def _teacher_targets(
    ...
    """Shooting teacher: sample K first-actions, roll forward H steps,
    distill the best first-action back into the policy."""
    ...
    # Clone simulator state for shooting rollouts
    act_hist_prev = (
        sim_ref.action_history[ridx, h - CONTEXT_LENGTH : h]
        .clone()
        .repeat_interleave(k, dim=0)
    )
    state_hist_prev = (
        sim_ref.state_history[ridx, h - CONTEXT_LENGTH : h, :]
        .clone()
        .repeat_interleave(k, dim=0)
    )
    pred_hist_prev = (
        sim_ref.current_lataccel_history[ridx, h - CONTEXT_LENGTH : h]
        .clone()
        .repeat_interleave(k, dim=0)
    )

    act_hist_ctrl = (
        _ring_to_seq(h_act32[ridx], hist_head).clone().repeat_interleave(k, dim=0)
    )
    lat_hist_ctrl = (
        _ring_to_seq(h_lat[ridx], hist_head).clone().repeat_interleave(k, dim=0)
    )
    ...
    with torch.inference_mode():
        ...
        for j in range(horizon):
            ...
            # Get policy mu (deterministic continuation for j>0, sampled for j==0)
            mu, sigma = ac.get_mu_sigma(obs_j)

            if j == 0:
                # Sample K first-actions from the Gaussian policy
                eps = torch.randn_like(mu)
                raw = mu + sigma * eps
                # Slot 0 = policy mean (base for comparison)
                raw[base_mask] = mu[base_mask]
                first_raw = raw.clone()
            else:
                # Deterministic continuation: use mu
                raw = mu

            squashed = torch.tanh(raw)
            prev_action = act_hist_ctrl[:, -1].double()
            current_before = current
            action = (prev_action + squashed.double() * ds).clamp(
                STEER_RANGE[0], STEER_RANGE[1]
            )

            # Roll forward through ONNX physics model (expected value)
            state_now = torch.stack([roll_j, v_j, a_j], dim=1).double()
            actions_in = torch.cat([act_hist_prev[:, 1:], action.unsqueeze(1)], dim=1)
            states_in = torch.cat(
                [state_hist_prev[:, 1:, :], state_now.unsqueeze(1)], dim=1
            )
            ...
            pred = expected.clamp(
                current_before - MAX_ACC_DELTA, current_before + MAX_ACC_DELTA
            )

            # Accumulate cost
            lat_cost = (target_j.double() - pred).pow(2) * (
                100.0 * LAT_ACCEL_COST_MULTIPLIER
            )
            ...
            pred_hist_prev = torch.cat(
                [pred_hist_prev[:, 1:], pred.unsqueeze(1)], dim=1
            )
            act_hist_ctrl = torch.cat(
                [act_hist_ctrl[:, 1:], action.float().unsqueeze(1)], dim=1
            )
            lat_hist_ctrl = torch.cat(
                [lat_hist_ctrl[:, 1:], current_before.float().unsqueeze(1)], dim=1
            )

    if first_raw is None:
        return None

    # Pick best first-action per route
    ...
    return (
        ...
        best_raw[keep].float().clone(),  # pre-tanh raw values
        weights.float().clone(),
    )


# ══════════════════════════════════════════════════════════════
#  Rollout (same as exp059 + teacher collection)
# ══════════════════════════════════════════════════════════════
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        # Collect teacher targets (only during training)
        if not deterministic:
...
# ══════════════════════════════════════════════════════════════
#  PPO with teacher distillation (Gaussian NLL)
# ══════════════════════════════════════════════════════════════
...
class PPO(base.PPO):
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        ...
        t_raw = gd["teacher_raw"]  # pre-tanh target values
        t_w = gd["teacher_w"]
        ...
        for idx in torch.randperm(len(t_obs), device="cuda").split(TEACHER_BS):
            mu, sigma = self.ac.get_mu_sigma(t_obs[idx])
            # Gaussian NLL on pre-tanh space, weighted by improvement
            nll = 0.5 * ((t_raw[idx] - mu) / sigma).pow(2) + sigma.log()
            loss = (nll * t_w[idx]).mean()
...
def train():
    ...
    if RESUME and BEST_PT.exists():
        ...
        # Try loading exp059 checkpoint for warm start
        exp059_pt = ROOT / "experiments" / "exp059_precision_tracking" / "best_model.pt"
        if exp059_pt.exists():
            ckpt = torch.load(exp059_pt, weights_only=False, map_location=DEV)
            ac.load_state_dict(ckpt["ac"])
            print(f"Warm-started from exp059: {exp059_pt}")
            print("Fresh optimizer + critic. Running critic warmup.")
        else:
    ...
    print(f"\nPPO+Teacher  csvs={CSVS_EPOCH}  epochs={MAX_EP}  dev={DEV}")
    ...
    torch.save(
        {"ac": ac.state_dict(), "delta_scale": cur_ds}, EXP_DIR / "final_model.pt"
    )
