# exp106 — PPO with physics-model entropy reward bonus
#
# Same as exp095/exp055, but adds a reward bonus for actions that make the
# physics model's output distribution more confident (lower entropy).
#
# Hypothesis: if the policy learns to take actions that produce sharp
# physics model predictions, the temp=0.8 sampling becomes more like temp=0.1,
# reducing the noise gap.
...
PHYS_ENT_COEF = float(
    os.getenv("PHYS_ENT_COEF", "1.0")
)  # physics entropy penalty weight
...
def rollout(
    ...
    # Physics entropy tracking: collect entropy of model's output distribution
    phys_entropy = torch.zeros((N, max_steps), dtype=torch.float32, device="cuda")
    ...
    def ctrl(step_idx, sim_ref):
        ...
        # Collect physics model entropy from previous step's prediction
        if si > 0 and not deterministic:
            probs = getattr(sim_ref.sim_model, "_last_probs_gpu", None)
            if probs is not None:
                ent = -(probs * torch.log(probs.clamp(min=1e-10))).sum(dim=-1)
                phys_entropy[:, si - 1] = ent
    ...
    # Collect final step's entropy
    if not deterministic and si > 0:
        probs = getattr(sim.sim_model, "_last_probs_gpu", None)
        if probs is not None:
            ent = -(probs * torch.log(probs.clamp(min=1e-10))).sum(dim=-1)
            phys_entropy[:, si - 1] = ent
    ...
    # Add physics entropy penalty: higher entropy = more noise = more penalty
    ent_penalty = phys_entropy[:, :S] * PHYS_ENT_COEF
    rew = (
        -(lat_r + jerk**2 * 100 + act_d**2 * ACT_SMOOTH + ent_penalty)
        / max(REWARD_SCALE, 1e-8)
    ).float()
...
class TrainingContext:
    ...
    def resume(self):
        RESET_OUTPUT = os.getenv("RESET_OUTPUT", "0") == "1"
        if RESUME and BEST_PT.exists():
            ckpt = torch.load(BEST_PT, weights_only=False, map_location=DEV)
            self.ac.load_state_dict(ckpt["ac"])
            if RESET_OUTPUT:
                # Reset actor output layer to restore exploration
                _ortho(self.ac.actor[-1], gain=0.01)
                # Reset critic fully for fresh value learning
                for layer in self.ac.critic:
                    _ortho(layer)
                _ortho(self.ac.critic[-1], gain=1.0)
                # Fresh optimizers + RMS
                self.ppo.pi_opt = optim.Adam(
                    self.ac.actor.parameters(), lr=PI_LR, eps=1e-5
                )
                self.ppo.vf_opt = optim.Adam(
                    self.ac.critic.parameters(), lr=VF_LR, eps=1e-5
                )
                self.ppo._rms = RunningMeanStd()
                self.warmup_off = 0  # re-enable critic warmup
                print("RESET_OUTPUT=1: actor output + critic reset, fresh optimizers")
            elif RESUME_OPT and "pi_opt" in ckpt:
...
def train():
    ...
    if COMPILE:
        ctx.ac.actor = torch.compile(
            ctx.ac.actor, mode="max-autotune-no-cudagraphs", dynamic=True
        )
        ctx.ac.critic = torch.compile(
            ctx.ac.critic, mode="max-autotune-no-cudagraphs", dynamic=True
        )
