#!/usr/bin/env python3
"""
Experiment 011: Multi-file PPO with Feynman simplicity
Architecture based on beautiful_lander.py
State: 4D = [error, error_integral, error_derivative, v_ego]
"""
import sys
sys.path.insert(0, '../..')
...
# Hyperparameters
STATE_DIM = 4  # Simple state: error, integral, derivative, v_ego
ACTION_DIM = 1
HIDDEN_DIM = 32  # Small network (Feynman simplicity)
TRUNK_LAYERS = 1
HEAD_LAYERS = 1  # Keep it simple
LR = 1e-3
GAMMA = 0.99
GAE_LAMBDA = 0.95
EPS_CLIP = 0.2
...
BATCH_SIZE = 2048
NUM_ENVS = 8
STEPS_PER_EPOCH = 20_000  # More steps to see more files
MAX_EPOCHS = 500  # Train longer
LOG_INTERVAL = 5

device = torch.device('cpu')

def build_state(target_lataccel, current_lataccel, state, future_plan, prev_error, error_integral):
    """Build simple 4D state: error, integral, derivative, v_ego"""
    error = target_lataccel - current_lataccel
    error_derivative = error - prev_error
    return np.array([error, error_integral, error_derivative, state.v_ego], dtype=np.float32)
...
class TinyPhysicsGymEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        obs = build_state(target, self.sim.current_lataccel, state, future_plan, self.prev_error, self.error_integral)
        return obs, {}
    
    def step(self, action):
        # Apply action
        action_clipped = np.clip(action[0], STEER_RANGE[0], STEER_RANGE[1])
        ...
        if not done:
            ...
            error = target - self.sim.current_lataccel
            self.error_integral += error
            self.error_integral = np.clip(self.error_integral, -14, 14)  # Anti-windup
            obs = build_state(target, self.sim.current_lataccel, state, future_plan, self.prev_error, self.error_integral)
...
def main():
    print("\n" + "="*60)
    print("Experiment 011: Multi-file PPO (Feynman simplicity)")
    print("="*60)
    print(f"State: {STATE_DIM}D (error, integral, derivative, v_ego)")
    print(f"Architecture: Trunk({TRUNK_LAYERS}) + Heads({HEAD_LAYERS}) × {HIDDEN_DIM}")
    print(f"Envs: {NUM_ENVS}, Steps/epoch: {STEPS_PER_EPOCH}, Max epochs: {MAX_EPOCHS}")
    print(f"LR: {LR}, Entropy: {ENTROPY_COEF}, K_epochs: {K_EPOCHS}")
    print("="*60 + "\n")
    
    # Setup
    model_path = "../../models/tinyphysics.onnx"
    all_files = sorted(glob.glob("../../data/*.csv"))[:100]  # 100 files
