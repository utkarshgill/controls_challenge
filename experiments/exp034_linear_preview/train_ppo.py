...
**The decisive test**: If a linear controller on 3 preview features can't beat PID,
then preview doesn't help in this simulator. If it does, we have the winning geometry.

Architecture: u = u_PID(e) + LPF(w1×f1 + w2×f2 + w3×f3)

Where:
- f1 = mean(Δlataccel[1:6])    # 0.1-0.6s near term
- f2 = mean(Δlataccel[10:25])  # 1.0-2.5s mid horizon
- f3 = mean(Δlataccel) / len   # long term curvature mass

Δlataccel[i] = future.lataccel[i] - future.lataccel[0]  # Relative to t+1, NOT current

Policy: PURE LINEAR (no hidden layers)
- Actor: 3 input → 3 weights → 1 output
- No MLP, no nonlinearity, no hidden state
- Just learns: w = [w1, w2, w3]

Low-pass: α = 0.05 (much stronger than before)

This is the **minimal hypothesis class** that can represent anticipation.
"""
import sys
...
# Task dimensions - MINIMAL LINEAR PREVIEW BASIS
STATE_DIM = 3   # [f1, f2, f3] - 3 canonical preview features only
...
class TinyPhysicsGymEnv:
    ...
    def _get_state(self):
        """Construct 3D MINIMAL PREVIEW BASIS
        
        Features:
        - f1: mean(Δlataccel[1:6])    # near term (0.1-0.6s)
        - f2: mean(Δlataccel[10:25])  # mid horizon (1.0-2.5s)  
        - f3: mean(Δlataccel) / len   # long term curvature mass
        
        Where Δlataccel[i] = future.lataccel[i] - future.lataccel[0]
        (relative to t+1, NOT current error or absolute target)
        
        GUARANTEES:
        - No error information
        - No current state correlation
        - Pure anticipatory signal
        - Cannot reconstruct PID reactive space
        """
        if self.sim is None or self.step_idx >= len(self.sim.data):
            return np.zeros(STATE_DIM, dtype=np.float32)
        
        # Get future lataccel plan
        future_lat = []
        for i in range(50):
            future_idx = self.step_idx + i
            if future_idx < len(self.sim.data):
                future_lat.append(self.sim.data['target_lataccel'].values[future_idx])
            else:
                future_lat.append(future_lat[-1] if future_lat else 0.0)
        
        # Compute differential features (relative to t+1, not current)
        if len(future_lat) > 0:
            baseline = future_lat[0]  # t+1 target
            delta = np.array([lat - baseline for lat in future_lat])
            
            # f1: Near term anticipation (0.1-0.6s)
            f1 = np.mean(delta[1:6]) if len(delta) > 6 else 0.0
            
            # f2: Mid horizon curvature (1.0-2.5s)
            f2 = np.mean(delta[10:25]) if len(delta) > 25 else 0.0
        ...
        # Normalize to reasonable scale (~[-1, 1])
        state = np.array([
            f1 / LATACCEL_SCALE,
            f2 / LATACCEL_SCALE,
            f3 / LATACCEL_SCALE
...
def train():
    print("="*80)
    print("Experiment 034: 3-Parameter Linear Preview Controller")
    print("="*80)
    print(f"Architecture: u = u_PID(e) + LPF(w1×f1 + w2×f2 + w3×f3)")
    print(f"  Policy: PURE LINEAR (3 weights only)")
    print(f"  ε (residual scale): {RESIDUAL_SCALE}")
    print(f"  Residual clip: ±{RESIDUAL_CLIP}")
    print(f"  Low-pass α: {LOWPASS_ALPHA} (very aggressive)")
