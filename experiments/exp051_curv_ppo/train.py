...
STATE_DIM, HIDDEN     = 156, 256        # 16 core + 40 hist + 50 curv + 50 a_ego
...
def build_obs_batch(target, current, roll_la, v_ego, a_ego,
    ...
    def _pad_future(attr):
        slc = fplan_data[attr][:, step_idx+1:end]
        fb = fallback[attr]
        if slc.shape[1] == 0:
            return np.repeat(fb[:, None], FUTURE_K, axis=1).astype(np.float32)
        elif slc.shape[1] < FUTURE_K:
            return np.concatenate([slc,
                np.repeat(slc[:, -1:], FUTURE_K - slc.shape[1], axis=1)], 1).astype(np.float32)
        return slc.astype(np.float32)

    f_lat  = _pad_future('target_lataccel')
