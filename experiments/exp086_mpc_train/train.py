# exp086 — PPO with train-time shooting MPC
#
# Same as exp055, but at each step during rollout collection,
# the controller samples MPC_N candidates from the policy, evaluates
# each over MPC_H steps of expected-value ONNX lookahead, and picks
# the lowest-cost candidate. PPO trains on the MPC-selected actions.
#
# At test time: clean policy inference, no MPC.
# The MPC only provides better action selection during training,
# giving PPO a stronger signal to learn from.
...
# ── Train-time MPC ────────────────────────────────────────────
MPC_N = int(os.getenv("MPC_N", "16"))  # candidates per route per step
MPC_H = int(os.getenv("MPC_H", "5"))  # lookahead horizon (expected-value ONNX)
MPC_TEMP = float(os.getenv("MPC_TEMP", "3.0"))  # sampling temperature for candidates
MPC_EVERY = int(os.getenv("MPC_EVERY", "5"))  # MPC epoch every N epochs
...
# ══════════════════════════════════════════════════════════════
#  Shooting MPC evaluator (expected-value ONNX, batched)
# ══════════════════════════════════════════════════════════════
...
@torch.no_grad()
def mpc_shoot_batched(a_p, b_p, prev_steer, sim_ref, dg, step_idx, ort_session, ds):
    """Shooting MPC for all N routes simultaneously.

    Samples MPC_N candidates per route from the policy Beta (at MPC_TEMP),
    evaluates each via H-step expected-value ONNX lookahead,
    returns the best action per route.

    a_p, b_p: (N,) Beta params from policy
    prev_steer: (N,) previous steer action
    sim_ref: BatchedSimulator reference (for histories)
    dg: data_gpu dict
    step_idx: current step
    ort_session: TRT ONNX session for expected-value lookahead
    ds: delta scale

    Returns: best_action (N,) float64, best_raw (N,) float32, logp (N,) float32
    """
    N = a_p.shape[0]
    K = MPC_N
    CL = CONTEXT_LENGTH

    # Sample K candidates per route from temperature-scaled Beta
    # Candidate 0 = deterministic mean
    a_t = (a_p / MPC_TEMP).clamp_min(0.01)
    b_t = (b_p / MPC_TEMP).clamp_min(0.01)

    # (N, K) samples
    mean_x = a_p / (a_p + b_p)  # (N,)
    samples = torch.distributions.Beta(
        a_t.unsqueeze(1).expand(-1, K), b_t.unsqueeze(1).expand(-1, K)
    ).sample()  # (N, K)
    samples[:, 0] = mean_x  # candidate 0 = mean

    raw_candidates = 2.0 * samples - 1.0  # (N, K)
    delta_candidates = raw_candidates * ds  # (N, K)
    action_candidates = (prev_steer.float().unsqueeze(1) + delta_candidates).clamp(
        STEER_RANGE[0], STEER_RANGE[1]
    )  # (N, K)

    # Build context from sim histories: (N, CL, ...)
    start = max(0, step_idx - CL + 1)
    act_hist = sim_ref.action_history[:, start:step_idx].float()  # (N, up_to_CL-1)
    state_hist = sim_ref.state_history[
        :, start : step_idx + 1, :3
    ].float()  # (N, up_to_CL, 3)
    ...
    # Pad if needed
    pad_act = CL - 1 - act_hist.shape[1]
    pad_state = CL - state_hist.shape[1]
    pad_pred = CL - pred_hist.shape[1]
    if pad_act > 0:
        act_hist = F.pad(act_hist, (pad_act, 0), value=0.0)
    if pad_state > 0:
        state_hist = F.pad(state_hist, (0, 0, pad_state, 0), value=0.0)
    if pad_pred > 0:
        pred_hist = F.pad(pred_hist, (pad_pred, 0), value=0.0)

    current_la = sim_ref.current_lataccel_history[
        :, max(0, step_idx - 1)
    ].float()  # (N,)

    # Tile for K candidates: (N*K, ...)
    act_h = act_hist.unsqueeze(1).expand(-1, K, -1).reshape(N * K, -1)  # (N*K, CL-1)
    state_h = (
        state_hist.unsqueeze(1).expand(-1, K, -1, -1).reshape(N * K, -1, 3)
    )  # (N*K, CL, 3)
    pred_h = pred_hist.unsqueeze(1).expand(-1, K, -1).reshape(N * K, -1)  # (N*K, CL)
    cur_la = current_la.unsqueeze(1).expand(-1, K).reshape(N * K)  # (N*K,)
    cand_actions = action_candidates.reshape(N * K)  # (N*K,)

    # Target lataccels for cost computation
    target_las = dg["target_lataccel"][:, step_idx : step_idx + MPC_H].float()  # (N, H)
    if target_las.shape[1] < MPC_H:
        target_las = F.pad(
            target_las, (0, MPC_H - target_las.shape[1]), mode="replicate"
        )
    target_tiled = (
        target_las.unsqueeze(1).expand(-1, K, -1).reshape(N * K, -1)
    )  # (N*K, H)

    # Future states for context updates
    fut_roll = dg["roll_lataccel"][:, step_idx : step_idx + MPC_H].float()
    fut_v = dg["v_ego"][:, step_idx : step_idx + MPC_H].float()
    fut_a = dg["a_ego"][:, step_idx : step_idx + MPC_H].float()
    for fut in (fut_roll, fut_v, fut_a):
        if fut.shape[1] < MPC_H:
            fut = F.pad(fut, (0, MPC_H - fut.shape[1]), mode="replicate")

    costs = torch.zeros(N * K, device="cuda")
    prev_la = cur_la.clone()

    for h in range(MPC_H):
        # Action for this lookahead step: candidate action for h=0, hold for h>0
        if h == 0:
            act = cand_actions
        else:
            act = cand_actions  # zero-order hold

        # Build full context: append action to history
        full_act = torch.cat([act_h, act.unsqueeze(1)], dim=1)  # (N*K, CL)

        # State context
        s_idx = min(step_idx + h, dg["roll_lataccel"].shape[1] - 1)
        new_state = torch.stack(
            [
                ...
                .expand(-1, K)
                .reshape(N * K),
                dg["v_ego"][:, s_idx].float().unsqueeze(1).expand(-1, K).reshape(N * K),
                dg["a_ego"][:, s_idx].float().unsqueeze(1).expand(-1, K).reshape(N * K),
            ],
            dim=-1,
        )  # (N*K, 3)
        full_state = torch.cat(
            [state_h[:, 1:, :], new_state.unsqueeze(1)], dim=1
        )  # (N*K, CL, 3)

        # Build model input: (N*K, CL, 4)
        states_input = torch.cat([full_act.unsqueeze(-1), full_state], dim=-1)

        # Tokenize predictions
        tokens = (
            torch.bucketize(pred_h.clamp(-5, 5), BINS_GPU, right=False)
            .clamp(0, VOCAB_SIZE - 1)
            .long()
        )

        # Run ONNX expected-value (chunked, same TRT session)
        NK = states_input.shape[0]
        CHUNK = 512
        all_pred = torch.empty(NK, device="cuda")
        states_np = states_input.cpu().numpy()
        tokens_np = tokens.cpu().numpy().astype(np.int64)
        for c0 in range(0, NK, CHUNK):
            c1 = min(c0 + CHUNK, NK)
            logits_np = ort_session.run(
                None,
                {
                    "states": states_np[c0:c1],
                    "tokens": tokens_np[c0:c1],
                },
            )[0]
            logits = torch.from_numpy(logits_np[:, -1, :]).to("cuda") / 0.8
            probs = F.softmax(logits, dim=-1)
            all_pred[c0:c1] = (probs * BINS_GPU.unsqueeze(0)).sum(dim=-1)
        pred_la = all_pred  # (N*K,)
        pred_la = pred_la.clamp(cur_la - MAX_ACC_DELTA, cur_la + MAX_ACC_DELTA)
        ...
        prev_la = pred_la
        act_h = full_act[:, 1:]  # shift
        state_h = full_state
        pred_h = torch.cat([pred_h[:, 1:], pred_la.unsqueeze(1)], dim=1)
        cur_la = pred_la

    # Pick best candidate per route
    costs_2d = costs.view(N, K)
    best_idx = costs_2d.argmin(dim=1)  # (N,)

    best_raw = raw_candidates[torch.arange(N, device="cuda"), best_idx]  # (N,)
    best_action = action_candidates[torch.arange(N, device="cuda"), best_idx]  # (N,)

    # Log-prob under the ORIGINAL policy (not temperature-scaled)
    best_x = samples[torch.arange(N, device="cuda"), best_idx].clamp(1e-6, 1 - 1e-6)
    orig_dist = torch.distributions.Beta(a_p, b_p)
    logp = orig_dist.log_prob(best_x)

    return best_action.double(), best_raw, logp
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        elif mpc_mode and step_idx >= CONTROL_START_IDX:
            # Shooting MPC: sample K candidates, evaluate H steps, pick best
            action, raw_policy, logp = mpc_shoot_batched(
                a_p, b_p, h_act[:, hist_head], sim_ref, dg, step_idx, ort_session, ds
            )
            delta = raw_policy.to(h_act.dtype) * ds
        else:
...
def train():
    ...
    print(
        ...
        f"\n  MPC: N={MPC_N} H={MPC_H} T={MPC_TEMP} every={MPC_EVERY}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        for pg in ppo.vf_opt.param_groups:
            pg["lr"] = vf_lr
        is_mpc = (epoch % MPC_EVERY == 0) and MPC_N > 1
        ...
        # BC toward MPC actions on MPC epochs
        bc_nll = 0.0
        if is_mpc and not co:
            obs_bc = res["obs"]
            raw_bc = res["raw"]
            x_t = ((raw_bc.unsqueeze(-1) + 1) / 2).clamp(1e-6, 1 - 1e-6)
            nll_sum, n_bc = 0.0, 0
            for idx in torch.randperm(len(obs_bc), device="cuda").split(MINI_BS):
                a_out = ac.actor(obs_bc[idx])
                a_c = F.softplus(a_out[..., 0]) + 1.0
                b_c = F.softplus(a_out[..., 1]) + 1.0
                nll = (
                    -torch.distributions.Beta(a_c, b_c)
                    .log_prob(x_t[idx].squeeze(-1))
                    .mean()
                )
                ppo.pi_opt.zero_grad(set_to_none=True)
                (nll * 0.1).backward()  # small weight so BC doesn't dominate PPO
                nn.utils.clip_grad_norm_(ac.actor.parameters(), 0.5)
                ppo.pi_opt.step()
                nll_sum += nll.item() * idx.numel()
                n_bc += idx.numel()
            bc_nll = nll_sum / max(1, n_bc)

        tu = time.time() - t1

        mpc_tag = f"  [MPC bc={bc_nll:.3f}]" if is_mpc else ""
        ...
        line = (
            ...
            f"  Δs={ds:.4f}  lr={info['lr']:.1e}  ⏱{t1 - t0:.0f}+{tu:.0f}s{phase}{mpc_tag}"
        )
