...
**The core insight**: PPO failed because observations were ABSOLUTE.
The plant is autoregressive and rate-limited. It operates on DELTAS.

**The one change**: Encode preview as deltas relative to CURRENT state (t).

Before (exp034): Δ[i] = future[i] - future[0]  (future-relative)
After  (exp036): Δ[i] = future[i] - current(t) (present-relative)

This gives PPO phase information:
- "Am I entering or exiting a curve?"
- "Am I ahead or behind the curvature?"
- "What is the rate of change?"

Without deltas: PPO explores locally-good actions that integrate badly over 400 steps.
With deltas: PPO sees phase and can coordinate with plant dynamics.

State encoding (200D):
- Δlataccel[i] = future.lataccel[i] - target_lataccel(t)   for i in [0..49]
- Δroll[i]     = future.roll[i]     - roll_lataccel(t)     for i in [0..49]
- Δv[i]        = future.v[i]        - v_ego(t)              for i in [0..49]
- Δa[i]        = future.a[i]        - a_ego(t)              for i in [0..49]

This is the minimal change that adds memory without adding memory.
"""
import sys
...
# Task dimensions - FULL DELTA-ENCODED PREVIEW
STATE_DIM = 200  # 4 signals × 50 steps: [Δlataccel, Δroll, Δv, Δa] all relative to current(t)
...
BATCH_SIZE = 2048
K_EPOCHS = 10
HIDDEN_DIM = 64   # Shallow MLP (200D input needs some compression)
ACTOR_LAYERS = 2  # 2 hidden layers (following beautiful_lander.py)
CRITIC_LAYERS = 2
PI_LR = 3e-4
VF_LR = 1e-3
GAMMA = 0.99
GAE_LAMBDA = 0.95
EPS_CLIP = 0.2
VF_COEF = 0.05  # Nerfed - still a POMDP
ENTROPY_COEF = 0.0  # ZERO - no exploration

# DELTA-ENCODED PREVIEW ARCHITECTURE (Experiment 036):
# 1. State: 200D delta-encoded preview (4 signals × 50 steps) - ALL relative to current(t)
# 2. Policy: Shallow MLP (200 → 64 → 64 → 1) - learns phase-aware anticipation
# 3. Low-pass: α=0.05 (aggressive filtering)
# 4. This tests if PPO needs phase information, not more power
...
class TinyPhysicsGymEnv:
    ...
    def _get_state(self):
        """Construct 200D DELTA-ENCODED PREVIEW
        
        CRITICAL CHANGE from exp034:
        - OLD: Δ[i] = future[i] - future[0]  (future-relative, hides phase)
        - NEW: Δ[i] = future[i] - current(t) (present-relative, reveals phase)
        
        This encodes "am I ahead/behind the curve?" which is phase information
        needed for autoregressive plant coordination.
        
        State = [Δlataccel[0..49], Δroll[0..49], Δv[0..49], Δa[0..49]]
        Where:
        - Δlataccel[i] = future.lataccel[i] - target_lataccel(t)
        ...
        # Current state (baselines for delta encoding)
        current_target = self.sim.target_lataccel_history[self.step_idx]
        current_state = self.sim.state_history[self.step_idx]
        current_roll = current_state.roll_lataccel
        current_v = current_state.v_ego
        current_a = current_state.a_ego
        ...
        for i in range(50):
            ...
            if future_idx < len(self.sim.data):
                ...
                future_a = self.sim.data['a_ego'].values[future_idx]
            else:
                # Pad with last valid value
                future_lat = delta_lataccel[-1] + current_target if delta_lataccel else current_target
                future_roll = delta_roll[-1] + current_roll if delta_roll else current_roll
                future_v = delta_v[-1] + current_v if delta_v else current_v
                future_a = delta_a[-1] + current_a if delta_a else current_a
            
            # Compute deltas relative to CURRENT state (t)
            delta_lataccel.append((future_lat - current_target) / LATACCEL_SCALE)
            delta_roll.append((future_roll - current_roll) / LATACCEL_SCALE)
            delta_v.append((future_v - current_v) / V_SCALE)
            delta_a.append((future_a - current_a) / A_SCALE)
        
        # Concatenate all deltas into 200D state
        state = np.concatenate([delta_lataccel, delta_roll, delta_v, delta_a], dtype=np.float32)
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, actor_layers, critic_layers):
        ...
        # Actor network: Shallow MLP (following beautiful_lander.py)
        # 200D delta-encoded input → 64 → 64 → 1
        ...
        # Very low noise (σ≈0.1) - minimal exploration
...
def train():
    print("="*80)
    print("Experiment 036: Delta-Encoded Preview (Phase-Aware PPO)")
    print("="*80)
    print(f"Architecture: u = u_PID(e) + LPF(π_θ(Δ_preview))")
    print(f"  State: 200D delta-encoded (Δ relative to current t)")
    print(f"  Policy: Shallow MLP ({STATE_DIM} → {HIDDEN_DIM} → {HIDDEN_DIM} → 1)")
