"""exp061 — clean test controller (no MPC at inference time).

This is the submission controller for exp061_mpc_teacher.
Architecture and obs layout are identical to exp055.
The model was trained with MPC-guided rollouts (shooting MPC at train time),
but test inference is pure policy: deterministic Beta mean, no search.
"""
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp061_mpc_teacher"
        ckpt = None
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        # Deterministic Beta mean — no MPC, no sampling
