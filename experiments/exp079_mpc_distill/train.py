# exp079 — PPO + MPC search distillation
#
# Fork of exp055 with one addition: every MPC_EVERY epochs, run
# temperature-widened best-of-K trajectory search over ALL steps
# (not just the first), then BC the actor toward winning actions.
#
# This addresses the σ-collapse problem: PPO at convergence has σ≈0.004,
# so all sampled actions are near-identical. The MPC epochs force diverse
# exploration across the full trajectory, find better action sequences,
# and distill them back into the policy.
#
# Key difference from exp076: temperature is applied to ALL steps,
# not just the first. This allows the search to discover anticipatory
# timing adjustments across the full episode.
...
MPC_N = int(
    os.getenv("MPC_N", "64")
)  # candidates per route (need many for real search)
MPC_SIGMA = float(
    os.getenv("MPC_SIGMA", "0.1")
)  # exploration noise σ in raw delta space [-1,1]
MPC_EVERY = int(os.getenv("MPC_EVERY", "5"))  # MPC epoch every N epochs
MPC_START = int(os.getenv("MPC_START", "0"))  # start MPC from this epoch
MPC_LR = float(os.getenv("MPC_LR", "3e-4"))  # NLL learning rate for distillation
MPC_ROUTES = int(
    os.getenv("MPC_ROUTES", "100")
)  # routes per MPC epoch (fewer, more candidates each)
...
# ══════════════════════════════════════════════════════════════
#  MPC search: best-of-K full-trajectory with temp-widened policy
# ══════════════════════════════════════════════════════════════


def mpc_search(csv_files, ac, mdl_path, ort_session, csv_cache, ds=DELTA_SCALE_MAX):
    """Run MPC_N candidates per route with Gaussian noise on ALL steps.

    Candidate 0 = deterministic policy mean (baseline).
    Candidates 1..MPC_N-1 = policy mean + N(0, MPC_SIGMA) on every step.
    Pick the lowest-cost candidate per route, return (obs, raw) for distillation.
    """
    ...
    def ctrl(step_idx, sim_ref):
        ...
        noise = torch.randn(N, device="cuda") * MPC_SIGMA
        raw_policy = (mean_raw + noise).clamp(-1.0, 1.0)

        # Candidate 0 per route = deterministic mean (no noise)
        raw_2d = raw_policy.view(n_routes, MPC_N)
        mean_2d = mean_raw.view(n_routes, MPC_N)
        raw_2d[:, 0] = mean_2d[:, 0]
        raw_policy = raw_2d.reshape(-1)
        ...
        if CONTROL_START_IDX <= step_idx < COST_END_IDX:
            all_obs[si] = obs_buf
            all_raw[si] = raw_policy.float()
            si += 1
        return action

    costs_all = sim.rollout(ctrl)["total_cost"]
    S = si

    # Pick winner per route
    costs_t = torch.from_numpy(costs_all).to("cuda").view(n_routes, MPC_N)
    winner = costs_t.argmin(dim=1)  # (n_routes,)
    winner_flat = torch.arange(n_routes, device="cuda") * MPC_N + winner

    # Extract winning obs and raw for all steps
    # all_obs: (S, N, OBS_DIM) -> want (S, n_routes, OBS_DIM) for winners
    win_obs = all_obs[:S, winner_flat, :]  # (S, n_routes, OBS_DIM)
    win_raw = all_raw[:S, winner_flat]  # (S, n_routes)

    # Flatten for BC: (S * n_routes, ...)
    obs_flat = win_obs.permute(1, 0, 2).reshape(-1, OBS_DIM)
    raw_flat = win_raw.T.reshape(-1)

    # Stats
    mean_costs = costs_t[:, 0]  # deterministic baseline (candidate 0)
    win_costs = costs_t.gather(1, winner.unsqueeze(1)).squeeze(1)
    improve = (mean_costs - win_costs).mean().item()
    frac_improved = (win_costs < mean_costs).float().mean().item()

    return dict(
        obs=obs_flat,
        raw=raw_flat,
        costs=costs_all[winner_flat.cpu().numpy()],
        mean_cost=mean_costs.mean().item(),
        win_cost=win_costs.mean().item(),
        improve=improve,
        frac_improved=frac_improved,
    )


def mpc_distill(ac, mpc_data, optimizer):
    """BC the actor toward MPC winning actions."""
    obs = mpc_data["obs"]
    raw = mpc_data["raw"]
    x_t = ((raw + 1) / 2).clamp(1e-6, 1 - 1e-6)
    n = obs.shape[0]
    nll_sum, n_total = 0.0, 0
    for idx in torch.randperm(n, device="cuda").split(MINI_BS):
        logits = ac.actor(obs[idx])
        a_p = F.softplus(logits[..., 0]) + 1.0
        b_p = F.softplus(logits[..., 1]) + 1.0
        dist = torch.distributions.Beta(a_p, b_p)
        nll = -dist.log_prob(x_t[idx]).mean()
...
def train():
    ...
    # MPC distillation optimizer (separate from PPO's)
    mpc_opt = optim.Adam(ac.actor.parameters(), lr=MPC_LR)
    ...
    print(
        ...
        f"\n  MPC: N={MPC_N} σ={MPC_SIGMA} every={MPC_EVERY} start={MPC_START} lr={MPC_LR} routes={MPC_ROUTES}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        is_mpc = epoch >= MPC_START and MPC_N > 1 and epoch % MPC_EVERY == 0

        t0 = time.time()
        if is_mpc:
            # ── MPC search + distill epoch ──
            n_routes = min(MPC_ROUTES, len(tr_f))
            batch = random.sample(tr_f, max(n_routes, 1))
            mpc_data = mpc_search(batch, ac, mdl_path, ort_sess, csv_cache, ds=ds)
            t1 = time.time()
            nll = mpc_distill(ac, mpc_data, mpc_opt)
            tu = time.time() - t1
            line = (
                f"E{epoch:3d}  [MPC]  mean={mpc_data['mean_cost']:.1f}  win={mpc_data['win_cost']:.1f}"
                f"  Δ={mpc_data['improve']:.2f}  frac={mpc_data['frac_improved']:.2f}"
