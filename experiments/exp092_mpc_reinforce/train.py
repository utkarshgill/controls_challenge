# exp092 — Per-step REINFORCE with MPC advantages
#
# At each step during rollout:
#   1. MPC evaluates K candidates over H steps (stochastic, GPU TRT)
#   2. MPC picks the best, records the per-step cost advantage
#   3. The advantage = (mean_cost - best_cost) across K candidates
#   4. REINFORCE: loss = -advantage * log_prob(chosen_action)
#
# No importance ratios. No critic. No GAE.
# The MPC provides clean per-step advantages directly.
# σ is fixed at MPC_SIGMA (hard floor, not learnable).
...
# ══════════════════════════════════════════════════════════════
#  MPC Physics (for shooting lookahead, same TRT session)
# ══════════════════════════════════════════════════════════════
...
mpc_phys = None  # initialized in train()
...
def rollout(
    ...
    if not deterministic:
        ...
        all_adv = torch.empty((max_steps, N), dtype=torch.float32, device="cuda")
    ...
    def ctrl(step_idx, sim_ref):
        ...
        if deterministic:
            ...
            if step_idx >= CONTROL_START_IDX and MPC_K > 1:
                # ── MPC shooting: K candidates, H-step lookahead ──
                K, H = MPC_K, MPC_H
                noise = torch.randn(N, K, device="cuda") * MPC_SIGMA
                raw_cand = (mean_raw.unsqueeze(1) + noise).clamp(-1.0, 1.0)  # (N, K)
                raw_cand[:, 0] = mean_raw  # candidate 0 = policy mean
                ...
                cur_la = sim_ref.current_lataccel_history[
                    :, max(0, step_idx - 1)
                ].float()
                ...
                for h in range(H):
                    ...
                    tokens = (
                        torch.bucketize(pr_h.clamp(-5, 5), BINS_GPU, right=False)
                        .clamp(0, VOCAB_SIZE - 1)
                        .long()
                    )

                    if h < rng_base.shape[0]:
                        rng_tiled = rng_base[h].unsqueeze(1).expand(-1, K).reshape(NK)
                    else:
                ...
                # Pick best and compute per-step advantage
                costs_2d = costs.view(N, K)
                best_idx = costs_2d.argmin(dim=1)
                best_cost = costs_2d[torch.arange(N, device="cuda"), best_idx]
                mean_cost = costs_2d.mean(dim=1)
                step_advantage = (mean_cost - best_cost).float()  # positive = good
                raw_policy = raw_cand[torch.arange(N, device="cuda"), best_idx]
            else:
                # Before control start: just use policy mean
                raw_policy = mean_raw
                step_advantage = torch.zeros(N, device="cuda")

            # Compute log-prob of the chosen action under the policy Beta
            x_for_logp = ((raw_policy + 1.0) / 2.0).clamp(1e-6, 1 - 1e-6)
            dist = torch.distributions.Beta(a_p, b_p)
            logp = dist.log_prob(x_for_logp)
    ...
    return dict(
        ...
        val_2d=all_val[:S].T,
        mpc_adv=all_adv[:S].T.reshape(-1),  # per-step MPC advantage
        rew=rew,
        done=dones,
        costs=costs,
    )
...
class PPO:
    ...
    def reinforce_update(self, gd, ds=DELTA_SCALE_MAX):
        """Per-step REINFORCE with MPC advantages. No importance ratios."""
        obs = gd["obs"]
        raw = gd["raw"].unsqueeze(-1)
        mpc_adv = gd["mpc_adv"]  # per-step advantage from MPC
        x_t = ((raw + 1) / 2).clamp(1e-6, 1 - 1e-6)
        ds = float(ds)

        # Normalize advantages
        mpc_adv = (mpc_adv - mpc_adv.mean()) / (mpc_adv.std() + 1e-8)

        pi_sum, ent_sum, n = 0.0, 0.0, 0
        for idx in torch.randperm(len(obs), device="cuda").split(MINI_BS):
            a_out = self.ac.actor(obs[idx])
            a_c = F.softplus(a_out[..., 0]) + 1.0
            b_c = F.softplus(a_out[..., 1]) + 1.0
...
def train():
    ...
    print(
        ...
        f"\n  MPC: K={MPC_K} H={MPC_H} σ={MPC_SIGMA}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        t1 = time.time()
        info = ppo.reinforce_update(res, ds=ds)
        tu = time.time() - t1
