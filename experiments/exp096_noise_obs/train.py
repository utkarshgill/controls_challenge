# exp096 — Kalman-filtered obs for noise robustness
#
# The policy sees Kalman-filtered current_lataccel instead of raw noisy value.
# Feature 1: kalman_lataccel / S_LAT (filtered estimate of true state)
# Feature 14: (raw - kalman) / S_LAT (noise estimate / innovation)
#
# The Kalman filter runs inside the controller — deployable.
# Adapts smoothing: trusts obs more during fast changes, smooths more during steady state.
# Train from scratch at temp=0.8.
...
# ── EMA filter ────────────────────────────────────────────────
KF_Q = float(os.getenv("KF_Q", "0.05"))  # process noise variance
KF_R = float(
    os.getenv("KF_R", "0.5")
)  # observation noise variance  # 0.3 = moderate smoothing (higher = trust new obs more)
...
def fill_obs(
    ...
    pl = h_lat[:, hist_head]
    buf[:, 0] = target / S_LAT
    if filtered_la is not None:
        buf[:, 1] = filtered_la / S_LAT  # Kalman-filtered lataccel
    else:
        buf[:, 1] = current / S_LAT
    buf[:, 2] = (target - current) / S_LAT  # error uses raw current
    buf[:, 3] = k_tgt / S_CURV
    buf[:, 4] = k_cur / S_CURV
    ...
    if filtered_la is not None:
        buf[:, 14] = (current - filtered_la) / S_LAT  # innovation (noise estimate)
    else:
        buf[:, 14] = fric
...
def rollout(
    ...
    kf_x = [None]  # Kalman state estimate
    kf_P = [None]  # Kalman error covariance
    ...
    def ctrl(step_idx, sim_ref):
        ...
        # Kalman filter: estimate true lataccel from noisy observations
        if kf_x[0] is None:
            kf_x[0] = cur32.clone()
            kf_P[0] = torch.full_like(cur32, KF_R)
        else:
            # Predict: state stays, uncertainty grows by Q
            P_pred = kf_P[0] + KF_Q
            # Update: blend prediction with observation
            K = P_pred / (P_pred + KF_R)  # Kalman gain (per-route)
            kf_x[0] = kf_x[0] + K * (cur32 - kf_x[0])
            kf_P[0] = (1.0 - K) * P_pred
        filtered_la = kf_x[0]
