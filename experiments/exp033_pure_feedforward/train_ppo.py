...
**Key insight**: Don't ask PPO to discover the nullspace. ENFORCE it architecturally.

Architecture: u = u_PID(e) + ε × π_θ(s_preview)

Where s_preview EXCLUDES all error terms:
- NO error
- NO error_integral  
- NO current_lataccel
...
- future_curvatures[50] (preview)

This GUARANTEES: Cov(π_θ, error) = 0

PID owns feedback.
Policy owns anticipation.

This is control decomposition, not ML tricks.
"""
import sys
...
# Task dimensions - PURE FEEDFORWARD (no error terms!)
STATE_DIM = 53  # [v_ego, a_ego, roll_lataccel, curvatures[50]] - NO ERROR TERMS
...
BATCH_SIZE = 2048
K_EPOCHS = 10
HIDDEN_DIM = 64  # Smaller - we're learning anticipation, not complex nonlinearities
ACTOR_LAYERS = 2  # Shallow - or even linear (set to 1 for pure linear)
CRITIC_LAYERS = 2
PI_LR = 3e-4
VF_LR = 1e-3
GAMMA = 0.99
...
ENTROPY_COEF = 0.0  # ZERO - no exploration, we're nudging a manifold

# PURE FEEDFORWARD ARCHITECTURE (Experiment 033):
# 1. NO error in state → Cov(policy, error) = 0 by construction
# 2. Shallow network → minimal nonlinearity, stays in stability tube
# 3. Zero entropy → no random exploration, deterministic refinement only
# 4. Low noise (σ≈0.1) → small perturbations around PID manifold
...
LOWPASS_ALPHA = 0.3   # Low-pass filter: action = α×new + (1-α)×old
...
class TinyPhysicsGymEnv:
    ...
    def _get_state(self):
        """Construct 53D PURE FEEDFORWARD state vector
        
        CRITICAL: NO error, NO error_integral, NO current_lataccel
        
        This ENFORCES nullspace by construction:
        - Policy cannot react to error (PID's territory)
        - Policy can only anticipate future curvature
        - Cov(policy_output, current_error) = 0 by design
        """
        ...
        for i in range(50):
            ...
            if future_idx < len(self.sim.data):
                ...
                curvature_normalized = curvature / CURVATURE_SCALE  # Scale to ~[-1, 1]
            else:
                curvature_normalized = 0.0
            future_curvatures.append(curvature_normalized)
        
        # PURE FEEDFORWARD state: vehicle dynamics + preview ONLY
        state = np.array([
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, actor_layers, critic_layers):
        ...
        # Actor network (outputs residual correction)
        ...
        # Initialize with very low noise for pure feedforward (σ≈0.1)
...
def train():
    print("="*80)
    print("Experiment 033: Pure Feedforward PPO (Nullspace by Construction)")
    print("="*80)
    print(f"Architecture: u = u_PID(e) + ε × π_θ(s_preview)")
    print(f"  State: NO error terms → Cov(π,e)=0 by design")
