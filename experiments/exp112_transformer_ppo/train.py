# exp112 — Transformer actor (encoder-decoder) with Beta PPO
#
# Based on exp055. Only change: MLP actor → transformer actor.
...
# ── Transformer actor dimensions ─────────────────────────────
T_D_MODEL = int(os.getenv('T_D_MODEL', '64'))
T_N_HEADS = int(os.getenv('T_N_HEADS', '4'))
T_N_ENC   = int(os.getenv('T_N_ENC', '2'))
T_N_DEC   = int(os.getenv('T_N_DEC', '2'))
T_DROPOUT = float(os.getenv('T_DROPOUT', '0.0'))


class TransformerActor(nn.Module):
    """Encoder-decoder transformer that takes (N, 256) obs, outputs (N, 2).

    Splits obs into:
      - core[0:16]:     16-dim rich features → conditioning
      - hist[16:56]:    (20, 2) action + lataccel history → decoder (causal)
      - plan[56:256]:   (50, 4) future plan → encoder (bidirectional)

    Decoder cross-attends to plan encoder.  Last decoder token + core → Beta params.
    """
    def __init__(self, d_model=T_D_MODEL, n_heads=T_N_HEADS,
                 n_enc=T_N_ENC, n_dec=T_N_DEC, dropout=T_DROPOUT):
        super().__init__()
        # Plan encoder: (50, 4) → bidirectional self-attention
        ...
        # History decoder: (20, 2) → causal self-attention + cross-attention to plan
        self.hist_proj = nn.Linear(2, d_model)
        self.hist_pos = nn.Parameter(torch.randn(1, HIST_LEN, d_model) * 0.02)
        dec_layer = nn.TransformerDecoderLayer(
            d_model=d_model, nhead=n_heads, dim_feedforward=d_model * 4,
            dropout=dropout, batch_first=True, norm_first=True)
        self.decoder = nn.TransformerDecoder(dec_layer, num_layers=n_dec)
        self.register_buffer('causal_mask',
            torch.triu(torch.ones(HIST_LEN, HIST_LEN), diagonal=1).bool())

        # Core conditioning: 16-dim → d_model
        self.core_proj = nn.Linear(C, d_model)

        # Output head → 2 (alpha, beta params)
        self.head = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.Linear(d_model, 2),
        )
        # Small init on final layer (like exp055's gain=0.01)
        nn.init.zeros_(self.head[-1].bias)
        nn.init.normal_(self.head[-1].weight, std=0.01)

    def _split_obs(self, obs):
        """Split flat obs into structured tensors. Reuse to avoid repeated work."""
        core = obs[:, :C]                                        # (N, 16)
        # History: (N, 20, 2) — contiguous view via reshape
        hist = obs[:, C:H2].reshape(-1, HIST_LEN, 2)            # act+lat interleaved? No, they're consecutive
        # Actually hist_act=[C:H1], hist_lat=[H1:H2] are separate blocks
        hist_act = obs[:, C:H1]                                  # (N, 20)
        hist_lat = obs[:, H1:H2]                                 # (N, 20)
        hist = torch.stack([hist_act, hist_lat], dim=-1)         # (N, 20, 2)
        # Plan: (N, 50, 4) — each feature is a 50-block, stack them
        plan = torch.stack([obs[:, F_LAT:F_ROLL], obs[:, F_ROLL:F_V],
                            obs[:, F_V:F_A], obs[:, F_A:]], dim=-1)
        return core, hist, plan

    def encode_plan(self, plan):
        """Encode plan once, reuse across calls. plan: (N, 50, 4)"""
        plan_emb = self.plan_proj(plan) + self.plan_pos
        return self.plan_encoder(plan_emb)                        # (N, 50, d)

    def forward(self, obs, plan_memory=None):
        """obs: (N, 256) → (N, 2). Optionally pass cached plan_memory."""
        core, hist, plan = self._split_obs(obs)

        if plan_memory is None:
            plan_memory = self.encode_plan(plan)                  # (N, 50, d)

        # Decode history (causal + cross-attention to plan)
        hist_emb = self.hist_proj(hist) + self.hist_pos
        mask = self.causal_mask[:HIST_LEN, :HIST_LEN]
        decoded = self.decoder(hist_emb, plan_memory,
                               tgt_mask=mask, tgt_is_causal=True) # (N, 20, d)

        # Last token + core conditioning → output
        last = decoded[:, -1, :]                                  # (N, d)
        core_cond = self.core_proj(core)                          # (N, d)
        return self.head(last + core_cond)                        # (N, 2)
...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        # Critic stays MLP (proven, simple)
...
BC_DATA_PATH = Path(os.getenv('BC_DATA',
    str(ROOT / 'experiments/exp110_mpc/checkpoints/bc_data.npz')))


def pretrain_bc(ac, all_csvs):
    if BC_DATA_PATH.exists():
        print(f"BC pretrain: loading MPC expert data from {BC_DATA_PATH}")
        d = np.load(BC_DATA_PATH)
        all_obs, all_raw = d['obs'], d['raw']
    else:
...
class PPO:
    ...
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        ...
        # Plan encoding done per-minibatch (full cache is 24GB, won't fit)
