...
Fix asymmetry by using symmetric activation functions (Tanh instead of ReLU)
ReLU(x) = max(0,x) is asymmetric → only passes positive values
Tanh(x) is symmetric → passes both positive and negative equally
...
class ActorCritic(nn.Module):
    """ActorCritic with Tanh activations for symmetry"""
    def __init__(self, state_dim, action_dim, hidden_dim, trunk_layers, head_layers):
        super().__init__()
        
        # Shared trunk (Tanh instead of ReLU for symmetry)
...
def main():
    print("\n" + "="*60)
    print("Experiment 006: PPO with Tanh Activations")
    print("="*60)
    print(f"State: {STATE_DIM}D normalized")
    print(f"Activation: Tanh (symmetric, not ReLU)")
    print(f"Data Aug: 50% horizontal flip")
