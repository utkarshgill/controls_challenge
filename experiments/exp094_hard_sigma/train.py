# exp094 — PPO with hard sigma floor via concentration clamping
#
# Identical to exp055, with ONE change: the Beta concentration (a_p + b_p)
# is hard-clamped to a maximum, preventing σ from collapsing below ~0.01.
# The mean is preserved. The policy's OWN distribution stays wide.
# All samples are on-policy. No importance ratio issues.
...
# ── Hard sigma floor ──────────────────────────────────────────
MAX_CONCENTRATION = float(os.getenv("MAX_CONC", "625"))  # a+b ≤ this → σ ≥ ~0.01
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        # Hard clamp: cap concentration to prevent σ collapse
        total = a_p + b_p
        scale = torch.clamp(MAX_CONCENTRATION / total, max=1.0)
        a_p = a_p * scale
        b_p = b_p * scale
...
def pretrain_bc(ac, all_csvs):
    ...
    for ep in range(BC_EPOCHS):
        ...
        for idx in torch.randperm(N).split(BC_BS):
            ...
            _total = a_p + b_p
            _scale = torch.clamp(MAX_CONCENTRATION / _total, max=1.0)
            a_p = a_p * _scale
            b_p = b_p * _scale
...
def train():
    ...
    print(
        ...
        f"\n  hard_σ: max_conc={MAX_CONCENTRATION}\n"
    )
