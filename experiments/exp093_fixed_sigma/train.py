# exp093 — PPO with fixed exploration sigma
#
# Identical to exp055, with ONE change: during collection,
# sample with fixed Gaussian σ instead of the policy's collapsed Beta σ.
# The policy still outputs Beta params (mean is learnable, σ can collapse),
# but ACTUAL exploration uses fixed σ=EXPLORE_SIGMA.
# Log-prob is computed under the Beta for PPO importance ratios.
...
EXPLORE_SIGMA = float(os.getenv("EXPLORE_SIGMA", "0.01"))  # in raw delta space [-1,1]
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        if deterministic:
            ...
            # Fixed exploration: Gaussian noise around policy mean
            mean_raw = 2.0 * a_p / (a_p + b_p) - 1.0
            raw_policy = (mean_raw + torch.randn_like(mean_raw) * EXPLORE_SIGMA).clamp(
                -1.0, 1.0
            )
            # Log-prob under the original Beta (for PPO importance ratios)
            x = ((raw_policy + 1.0) / 2.0).clamp(1e-6, 1 - 1e-6)
            logp = torch.distributions.Beta(a_p, b_p).log_prob(x)
