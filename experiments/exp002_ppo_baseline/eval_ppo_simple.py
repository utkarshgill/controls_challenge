#!/usr/bin/env python3
"""
Simple sequential evaluation of PPO model (no multiprocessing).
"""
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("Evaluating PPO Parallel Model")
    print("="*60)
    ...
    actor_critic.load_state_dict(torch.load('ppo_parallel_best.pth', map_location=device))
    actor_critic.eval()
    print("✅ Model loaded")
    ...
    # Evaluate
    print(f"\nEvaluating on {len(val_files[:100])} validation files...")
    costs = []
    for data_file in tqdm(val_files[:100]):
        controller = PPOController(actor_critic)
        sim = TinyPhysicsSimulator(physics_model, data_file, controller=controller, debug=False)
        sim.rollout()
    ...
    print("\n" + "="*60)
    print(f"PPO Parallel Results:")
    print(f"  Mean:   {np.mean(costs):.2f} ± {np.std(costs):.2f}")
    ...
    print("\n" + "="*60)
    print("Comparison:")
    print(f"  BC baseline:  ~84 cost")
    print(f"  PPO current:  {np.mean(costs):.2f} cost")
    print(f"  Target:       <45 cost")
    print("="*60)
