# exp082 — PPO + train-time MPC (structured per-copy offsets)
#
# Fork of exp055 with one key change: the K=10 copies per route aren't
# just stochastic Beta samples (which are all identical at σ=0.003).
# Instead, each copy gets a deterministic offset to the policy mean:
#   copy 0: deterministic mean (baseline)
#   copies 1-9: mean + offset[k] where offsets span [-OFFSET_MAX, +OFFSET_MAX]
#
# After rollout, the best copy per route is identified. If a non-baseline
# copy wins, its actions provide a BC target for the actor.
# PPO still runs on all copies with proper log-probs.
#
# This gives PPO real signal: the offsets create meaningfully different
# trajectories, and the batch-of-batch variance reduction identifies
# which offset direction helps on each route.
...
# ── MPC offsets ───────────────────────────────────────────────
OFFSET_MAX = float(
    os.getenv("OFFSET_MAX", "0.05")
)  # max offset in raw delta space [-1,1]
BC_MPC_COEF = float(
    os.getenv("BC_MPC_COEF", "1.0")
)  # weight of BC loss from MPC winners
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        if deterministic:
            ...
            # ── MPC offsets for structured exploration ──
            # Copy 0: Beta sample (for PPO — needs valid log-probs)
            # Copies 1..K-1: deterministic mean + fixed offsets (for MPC search only)
            K = SAMPLES_PER_ROUTE
            copy_idx = torch.arange(N, device="cuda") % K

            # Beta sample for copy 0
            dist = torch.distributions.Beta(a_p, b_p)
            x = dist.sample()
            raw_sampled = 2.0 * x - 1.0
            logp_sampled = dist.log_prob(x)

            # Offsets for copies 1..K-1
            offsets = torch.linspace(-OFFSET_MAX, OFFSET_MAX, K - 1, device="cuda")
            offsets = torch.cat([torch.zeros(1, device="cuda"), offsets])  # (K,)
            raw_offset = (mean_raw + offsets[copy_idx]).clamp(-1.0, 1.0)

            # Use sampled actions for copy 0, offset actions for copies 1-9
            is_copy0 = copy_idx == 0
            raw_policy = torch.where(is_copy0, raw_sampled, raw_offset)

            # Log-prob: only meaningful for copy 0. For offset copies, set to 0
            # (they won't be used in PPO update — filtered out below)
            x_for_logp = ((raw_policy + 1.0) / 2.0).clamp(1e-6, 1 - 1e-6)
            logp = torch.where(is_copy0, logp_sampled, torch.zeros_like(logp_sampled))
    ...
    # ── MPC winner identification ──
    K = SAMPLES_PER_ROUTE
    n_routes = N // K
    costs_2d = torch.from_numpy(costs).to("cuda").view(n_routes, K)
    winner_per_route = costs_2d.argmin(dim=1)
    winner_flat = torch.arange(n_routes, device="cuda") * K + winner_per_route

    # Extract winning obs and raw for BC
    win_obs = all_obs[:S, winner_flat, :]
    win_raw = all_raw[:S, winner_flat]

    # Stats
    baseline_costs = costs_2d[:, 0]
    winner_costs = costs_2d.gather(1, winner_per_route.unsqueeze(1)).squeeze(1)
    mpc_improve = (baseline_costs - winner_costs).mean().item()
    mpc_frac = (winner_per_route != 0).float().mean().item()

    # ── Separate PPO data (copy 0 only) from MPC data ──
    # Copy-0 indices: 0, K, 2K, ...
    copy0_idx = torch.arange(n_routes, device="cuda") * K  # (n_routes,)
    ppo_obs = all_obs[:S, copy0_idx, :]  # (S, n_routes, OBS_DIM)
    ppo_raw = all_raw[:S, copy0_idx]  # (S, n_routes)
    ppo_logp = all_logp[:S, copy0_idx]  # (S, n_routes)
    ppo_val = all_val[:S, copy0_idx]  # (S, n_routes)
    ppo_rew = rew[copy0_idx]  # (n_routes, S)
    ppo_done = dones[copy0_idx]  # (n_routes, S)

    return dict(
        # PPO data: only copy-0 (Beta-sampled, valid log-probs)
        obs=ppo_obs.permute(1, 0, 2).reshape(-1, OBS_DIM),
        raw=ppo_raw.T.reshape(-1),
        old_logp=ppo_logp.T.reshape(-1),
        val_2d=ppo_val.T,
        rew=ppo_rew,
        done=ppo_done,
        costs=costs,
        # MPC data: winners from all copies
        mpc_obs=win_obs.permute(1, 0, 2).reshape(-1, OBS_DIM),
        mpc_raw=win_raw.T.reshape(-1),
...
class PPO:
    ...
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        ...
        # Note: PPO data is copy-0 only (1 per route), so no batch-of-batch variance reduction.
        # The MPC search provides the cross-copy comparison via BC instead.
    ...
    def bc_mpc_update(self, mpc_obs, mpc_raw, ds=DELTA_SCALE_MAX):
        """BC toward MPC winner actions. One pass through the data."""
        if mpc_obs is None or len(mpc_obs) == 0:
            return 0.0
        x_t = ((mpc_raw.unsqueeze(-1) + 1) / 2).clamp(1e-6, 1 - 1e-6)
        ds = float(ds)
        nll_sum, n = 0.0, 0
        for idx in torch.randperm(len(mpc_obs), device="cuda").split(MINI_BS):
            a_out = self.ac.actor(mpc_obs[idx])
            a_c = F.softplus(a_out[..., 0]) + 1.0
            b_c = F.softplus(a_out[..., 1]) + 1.0
            dist = torch.distributions.Beta(a_c, b_c)
            nll = -dist.log_prob(x_t[idx].squeeze(-1)).mean()
            loss = nll * BC_MPC_COEF
            self.pi_opt.zero_grad(set_to_none=True)
            loss.backward()
            nn.utils.clip_grad_norm_(self.ac.actor.parameters(), 0.5)
            self.pi_opt.step()
...
def train():
    ...
    print(
        ...
        f"\n  MPC: offset_max={OFFSET_MAX}  bc_coef={BC_MPC_COEF}\n"
    )
    ...
    for epoch in range(MAX_EP):
        ...
        # BC toward MPC winners (only if non-baseline copies won on some routes)
        bc_nll = 0.0
        if not co and res.get("mpc_frac", 0) > 0:
            bc_nll = ppo.bc_mpc_update(res["mpc_obs"], res["mpc_raw"], ds=ds)
        tu = time.time() - t1

        phase = "  [critic warmup]" if co else ""
        mpc_str = f"  mpc_Δ={res.get('mpc_improve', 0):.2f} f={res.get('mpc_frac', 0):.2f} bc={bc_nll:.3f}"
        line = (
            ...
            f"  Δs={ds:.4f}  lr={info['lr']:.1e}{mpc_str}  ⏱{t1 - t0:.0f}+{tu:.0f}s{phase}"
        )
