...
from train import LinearActorCritic, build_state
...
# Load network
actor_critic = LinearActorCritic(state_dim=3, action_dim=1)
...
for file in files:
    ...
    while sim.step_idx < len(sim.data) - 50:
        ...
        obs = build_state(target, current_lataccel, state, prev_error, error_integral)
...
print(f"exp007 (Linear, 3 feat): {np.mean(costs):.1f}")
