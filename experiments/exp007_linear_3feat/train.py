"""
exp007: Linear model - just learn PID gains
State: [error, error_integral, error_derivative] (3 features)
Network: Linear(3, 1) - no hidden layers
"""
import os
import sys
import time
...
def build_state(target_lataccel, current_lataccel, state, prev_error, error_integral):
    """Build 3-feature state: [error, error_integral, error_derivative]"""
    error = target_lataccel - current_lataccel
    error_derivative = error - prev_error
    return np.array([error, error_integral, error_derivative], dtype=np.float32)

# ===== Linear Actor-Critic =====
class LinearActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        # Just linear layers - no hidden units!
...
def train():
    print("=" * 60)
    print("EXP007: Linear Model (3 features)")
    print("State: [error, error_integral, error_derivative]")
    print("Network: Linear(3, 1) - NO hidden layers")
    print("=" * 60)
    ...
    actor_critic = LinearActorCritic(state_dim=3, action_dim=1).to(DEVICE)
