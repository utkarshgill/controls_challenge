# exp089 — Hierarchical RL-MPPI (Hori et al.)
#
# Two-layer architecture:
#   High-level: RL policy outputs a lataccel offset (how much to shift the target)
#   Low-level:  MPPI searches for the steer action that tracks (target + offset)
#               using K candidates evaluated via the ONNX expected-value physics
#
# PPO trains the high-level policy on the real sim cost.
# MPPI handles the steer→lataccel dynamics. The policy only learns planning.
#
# At test time: same architecture. Policy outputs offset, MPPI finds steer.
# MPPI is cheap: K candidates × 1 batched ONNX call per step.
...
# ── MPPI low-level control ────────────────────────────────────
MPPI_K = int(os.getenv("MPPI_K", "16"))  # candidate steer actions per step
MPPI_STD = float(os.getenv("MPPI_STD", "0.1"))  # std for candidate steer perturbations
MPPI_TEMP = float(os.getenv("MPPI_TEMP", "1.0"))  # MPPI temperature
OFFSET_SCALE = float(
    os.getenv("OFFSET_SCALE", "0.5")
)  # max offset magnitude in lataccel space
...
# ══════════════════════════════════════════════════════════════
#  MPPI low-level steer search (batched, uses TRT ONNX)
# ══════════════════════════════════════════════════════════════
...
@torch.no_grad()
def mppi_steer_search(desired_lataccel, prev_steer, sim_ref, step_idx, ort_session):
    """Find the best steer action to track desired_lataccel for each of N routes.

    Uses MPPI: sample K candidate steer actions, evaluate 1 step via expected-value
    ONNX, pick the one whose predicted lataccel is closest to desired_lataccel.

    desired_lataccel: (N,) — target lataccel for each route
    prev_steer: (N,) — previous steer action
    sim_ref: BatchedSimulator — for context histories
    step_idx: current step
    ort_session: TRT ONNX session

    Returns: best_steer (N,) float64
    """
    N = desired_lataccel.shape[0]
    K = MPPI_K
    CL = CONTEXT_LENGTH

    # Generate K candidate steer actions per route: (N, K)
    # Centered on prev_steer with Gaussian perturbations
    noise = torch.randn(N, K, device="cuda") * MPPI_STD
    candidates = prev_steer.float().unsqueeze(1) + noise  # (N, K)
    candidates[:, 0] = prev_steer.float()  # always include prev_steer as a candidate
    candidates = candidates.clamp(STEER_RANGE[0], STEER_RANGE[1])

    # Build context from sim histories
    start = max(0, step_idx - CL + 1)
    act_hist = sim_ref.action_history[:, start:step_idx].float()  # (N, ≤CL-1)
    ...
    pad_a = CL - 1 - act_hist.shape[1]
    pad_s = CL - state_hist.shape[1]
    pad_p = CL - pred_hist.shape[1]
    if pad_a > 0:
        act_hist = F.pad(act_hist, (pad_a, 0), value=0.0)
    if pad_s > 0:
        state_hist = F.pad(state_hist, (0, 0, pad_s, 0), value=0.0)
    if pad_p > 0:
        pred_hist = F.pad(pred_hist, (pad_p, 0), value=0.0)

    # Tile for K candidates: (N*K, ...)
    act_h = act_hist.unsqueeze(1).expand(-1, K, -1).reshape(N * K, -1)
    state_h = state_hist.unsqueeze(1).expand(-1, K, -1, -1).reshape(N * K, CL, 3)
    pred_h = pred_hist.unsqueeze(1).expand(-1, K, -1).reshape(N * K, -1)
    cand_flat = candidates.reshape(N * K)  # (N*K,)

    # Append candidate action to history
    act_ctx = torch.cat([act_h, cand_flat.unsqueeze(1)], dim=1)  # (N*K, CL)

    # State context: use current step's data from sim
    # state_hist already has CL entries up to step_idx; just use it
    # But we need the last entry to match the current step's roll/v/a
    # state_hist[:, -1, :] is from step_idx, which is correct

    # Build (N*K, CL, 4): [action, roll_la, v_ego, a_ego]
    full_states = torch.cat([act_ctx.unsqueeze(-1), state_h], dim=-1)
    ...
    # Run ONNX expected-value (chunked if needed)
    NK = N * K
    CHUNK = 5000
    all_pred = torch.empty(NK, device="cuda")
    states_np = full_states.cpu().numpy()
    ...
    for c0 in range(0, NK, CHUNK):
        ...
        logits = torch.from_numpy(logits_np[:, -1, :]).to("cuda").float() / 0.8
        probs = F.softmax(logits, dim=-1)
        all_pred[c0:c1] = (probs * BINS_GPU.unsqueeze(0)).sum(dim=-1)

    # Clamp predictions
    cur_la = sim_ref.current_lataccel_history[:, max(0, step_idx - 1)].float()
    cur_la_tiled = cur_la.unsqueeze(1).expand(-1, K).reshape(NK)
    all_pred = all_pred.clamp(
        cur_la_tiled - MAX_ACC_DELTA, cur_la_tiled + MAX_ACC_DELTA
    )

    # Score: how close is predicted lataccel to desired?
    pred_2d = all_pred.view(N, K)
    desired_2d = desired_lataccel.unsqueeze(1).expand(-1, K)
    cost = (pred_2d - desired_2d) ** 2

    # MPPI-style weighted selection
    weights = F.softmax(-cost / MPPI_TEMP, dim=1)  # (N, K)
    best_steer = (weights * candidates).sum(dim=1)  # weighted mean
    best_steer = best_steer.clamp(STEER_RANGE[0], STEER_RANGE[1])

    return best_steer.double()
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        # High-level: policy outputs a lataccel offset
        if deterministic:
        ...
        offset = raw_policy * OFFSET_SCALE  # offset in lataccel space
        desired_lataccel = (target.float() + offset.float()).clamp(
            LATACCEL_RANGE[0], LATACCEL_RANGE[1]
        )

        # Low-level: MPPI finds steer action to track desired lataccel
        action = mppi_steer_search(
            desired_lataccel, h_act[:, hist_head], sim_ref, step_idx, ort_session
        )
...
def train():
    ...
    print(
        ...
        f"\n  MPPI: K={MPPI_K} std={MPPI_STD} temp={MPPI_TEMP} offset_scale={OFFSET_SCALE}\n"
    )
