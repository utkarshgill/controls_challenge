"""
exp048 v5 — Delta-Action PPO
=============================
54 dims: error, v_ego, prev_act, κ_now, κ_future×50
Tanh activations.  Learned σ (like beautiful_lander).  ENT=0.001.
Per-trajectory advantage normalization (removes inter-CSV variance).
"""
...
STATE_DIM   = 54        # error, v_ego, prev_act, κ_now, κ_future×50
HIDDEN      = 128
A_LAYERS    = 3
C_LAYERS    = 2
FUTURE_K    = 50
...
PI_LR       = 3e-5
VF_LR       = 1e-4
GAMMA       = 0.95      # short horizon — steer affects next ~20 lataccels
LAMDA       = 0.95
K_EPOCHS    = 10
EPS_CLIP    = 0.2
VF_COEF     = 0.5
ENT_COEF    = 0.001     # same as beautiful_lander
MINI_BS     = 5000
...
# [error/3, v/30, prev_act/2, κ×100 (×51)]
OBS_SCALE   = np.array([1/3, 1/30, 1/2] + [100.0]*51, dtype=np.float32)
OBS_SCALE_T = torch.tensor(OBS_SCALE)

EXP_DIR = Path(__file__).parent
TMP     = EXP_DIR / '.ckpt.pt'
BC_CKPT = EXP_DIR / 'bc_init.pt'
...
def build_obs(target, current, state, fplan, prev_act):
    obs = np.empty(STATE_DIM, np.float32)
    obs[0]    = target - current
    obs[1]    = state.v_ego
    obs[2]    = prev_act
    obs[3]    = _kappa(target, state.roll_lataccel, state.v_ego)
    obs[4:54] = _future_kappa(fplan, target, state)
    return obs


# ── Network (Tanh activations, learned log_std) ──────────────
...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.log_std = nn.Parameter(torch.zeros(1))   # σ₀=1.0, learned
...
class PIDRecorder(BaseController):
    def __init__(self):
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        self.n += 1
        e = target_lataccel - current_lataccel
        self.ei += e; de = e - self.pe; self.pe = e
        pid = float(np.clip(self.p*e + self.i*self.ei + self.d*de, *STEER_RANGE))
        ...
        obs = build_obs(target_lataccel, current_lataccel, state,
                        future_plan, self.prev_act)
        delta = np.clip(pid - self.prev_act, -MAX_DELTA, MAX_DELTA)
        self.prev_act = pid

        if self.n <= WARMUP_N + SCORED_N:
            self.bc.append((obs, delta / DELTA_SCALE))
        return pid
...
# ── PPO (learned σ, per-trajectory advantage norm) ───────────
...
class PPO:
    ...
    def gae(all_r, all_v, all_d):
        ...
        return (a - a.mean()) / (a.std() + 1e-8), r   # global normalization
    ...
    def update(self, all_obs, all_raw, all_rew, all_val, all_done):
        ...
        for _ in range(K_EPOCHS):
            for idx in torch.randperm(N).split(MINI_BS):
                ...
                vf_loss = F.mse_loss(val.squeeze(-1), ret_t[idx])
...
def _bc_worker(args):
    ...
    return (np.array([d[0] for d in ctrl.bc], np.float32),
            np.array([d[1] for d in ctrl.bc], np.float32))


# ── Training (beautiful_lander pattern) ──────────────────────
...
class Ctx:
    def __init__(self):
        ...
        self.tr_f = all_f[:N_TRAIN]
        self.va_f = all_f[N_TRAIN:N_TRAIN + N_VAL]
        self.best = float('inf')
        self.best_ep = -1
...
def bc_warmstart(ctx):
    print("Phase 1: BC from PID")
    t0 = time.time()
    args = [(str(f), str(ctx.mdl)) for f in ctx.tr_f[:BC_FILES]]
    res = process_map(_bc_worker, args, max_workers=WORKERS,
                      chunksize=10, desc="Collecting PID")
    obs = np.concatenate([r[0] for r in res])
    tgt = np.concatenate([r[1] for r in res])
    print(f"  {len(obs):,} samples in {time.time()-t0:.0f}s")
    print(f"  raw target: mean={tgt.mean():.3f}  std={tgt.std():.3f}")

    obs_t = torch.FloatTensor(obs) * OBS_SCALE_T
    tgt_t = torch.FloatTensor(tgt)
    opt = optim.Adam(ctx.ac.actor.parameters(), lr=BC_LR)

    for ep in range(BC_EP):
        tot, nb = 0.0, 0
        for idx in torch.randperm(len(obs_t)).split(MINI_BS):
            pred = ctx.ac.actor(obs_t[idx]).squeeze(-1)
            loss = F.mse_loss(pred, tgt_t[idx])
            opt.zero_grad(); loss.backward(); opt.step()
            tot += loss.item(); nb += 1
        if ep % 10 == 0 or ep == BC_EP - 1:
            print(f"  BC {ep:3d}/{BC_EP}  loss={tot/nb:.6f}")
    print()
...
def train():
    ...
    # ── Phase 1: BC ──
    if BC_CKPT.exists():
        print(f"Loading BC: {BC_CKPT}")
        ctx.ac.load_state_dict(
            torch.load(BC_CKPT, weights_only=False, map_location='cpu'))
    else:
        bc_warmstart(ctx)
        torch.save(ctx.ac.state_dict(), BC_CKPT)

    bc_m, bc_s = evaluate(ctx.ac, ctx.mdl, ctx.va_f)
    ctx.best, ctx.best_ep = bc_m, 'BC'
    print(f"BC baseline: {bc_m:.1f} ± {bc_s:.1f}\n")

    # ── Phase 2: PPO ──
    # σ₀=0.5 — the only setting that learned (v2). δσ=0.05/step, drift≈1.0/episode.
    ctx.ac.log_std.data.fill_(-0.7)   # σ₀ ≈ 0.5
    print(f"PPO  csvs={CSVS_EPOCH}  epochs={MAX_EP}  workers={WORKERS}")
