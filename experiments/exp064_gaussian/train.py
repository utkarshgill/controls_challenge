# exp064 — Gaussian policy with learnable log_std
#
# Fork of exp055 (gold standard, world #3).
# Only change: Beta distribution → Gaussian with learned log_std parameter.
# Actor outputs mean (tanh-squashed to [-1,1]), log_std is a separate learnable
# scalar (not state-dependent). This gives unbounded gradient flow through the
# mean — no Beta concentration parameter indirection.
...
class ActorCritic(nn.Module):
    def __init__(self):
        super().__init__()
        # Actor outputs 1 value: mean of raw delta in [-1,1] (via tanh)
        ...
        # Learnable log_std — single scalar, shared across all states
        self.log_std = nn.Parameter(torch.tensor(-1.0))  # init σ ≈ 0.37
...
def rollout(
    ...
    def ctrl(step_idx, sim_ref):
        ...
        with torch.inference_mode():
            mean_raw = ac.actor(obs_buf).squeeze(-1)
            ...
            if deterministic:
                ...
                std = ac.log_std.exp().clamp(min=1e-4)
                noise = torch.randn_like(mean) * std
                raw_policy = (mean + noise).clamp(-1.0, 1.0)
                logp = (
                    -0.5 * ((raw_policy - mean) / std) ** 2
                    - ac.log_std
                    - 0.5 * np.log(2 * np.pi)
                )
...
def pretrain_bc(ac, all_csvs):
    ...
    # Optimise both actor network AND log_std during BC
    opt = optim.AdamW(
        list(ac.actor.parameters()) + [ac.log_std], lr=BC_LR, weight_decay=1e-4
    )
    ...
    for ep in range(BC_EPOCHS):
        ...
        for idx in torch.randperm(N).split(BC_BS):
            mean_raw = ac.actor(obs_t[idx]).squeeze(-1)
            mean = torch.tanh(mean_raw)
            std = ac.log_std.exp().clamp(min=1e-4)
            # Gaussian NLL on raw targets ∈ [-1, 1]
            loss = (
                0.5 * ((raw_t[idx] - mean) / std) ** 2
                + ac.log_std
                + 0.5 * np.log(2 * np.pi)
            ).mean()
            opt.zero_grad()
            ...
            nn.utils.clip_grad_norm_(
                list(ac.actor.parameters()) + [ac.log_std], BC_GRAD_CLIP
            )
            opt.step()
            total += loss.item()
            nb += 1
...
class PPO:
    def __init__(self, ac):
        ...
        self.pi_opt = optim.Adam(
            list(ac.actor.parameters()) + [ac.log_std], lr=PI_LR, eps=1e-5
        )
        self.vf_opt = optim.Adam(ac.critic.parameters(), lr=VF_LR, eps=1e-5)
        self._rms = RunningMeanStd()

    @staticmethod
    def _gauss_logp(raw, mean, log_std):
        """Gaussian log prob: raw ∈ [-1,1], mean from tanh(net), log_std scalar."""
        std = log_std.exp().clamp(min=1e-4)
        return -0.5 * ((raw - mean) / std) ** 2 - log_std - 0.5 * np.log(2 * np.pi)
    ...
    def update(self, gd, critic_only=False, ds=DELTA_SCALE_MAX):
        ...
        with torch.no_grad():
            if "old_logp" in gd:
                ...
                old_mean = torch.tanh(self.ac.actor(obs).squeeze(-1))
                old_lp = self._gauss_logp(raw, old_mean, self.ac.log_std)
        ...
        for _ in range(K_EPOCHS):
            for idx in torch.randperm(len(obs), device="cuda").split(MINI_BS):
                ...
                if critic_only:
                    ...
                    mean_raw = self.ac.actor(obs[idx]).squeeze(-1)
                    mean = torch.tanh(mean_raw)
                    lp = self._gauss_logp(raw[idx], mean, self.ac.log_std)
                    ratio = (lp - old_lp[idx]).exp()
                    pi_loss = -torch.min(
                        ratio * mb_adv, ratio.clamp(1 - EPS_CLIP, 1 + EPS_CLIP) * mb_adv
                    ).mean()
                    # Gaussian entropy: 0.5 * ln(2πe * σ²)
                    ent = (0.5 + 0.5 * np.log(2 * np.pi) + self.ac.log_std).squeeze()
                    sigma_eff_mb = self.ac.log_std.exp() * ds
        ...
        with torch.no_grad():
            sigma_raw = self.ac.log_std.exp().item()
