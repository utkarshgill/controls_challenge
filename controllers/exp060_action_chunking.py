...
CHUNK_K = 5
STATE_DIM, HIDDEN = 258, 256
A_LAYERS = 4
DELTA_SCALE_DEFAULT = 0.25
LOG_SIGMA_MIN = -4.0
LOG_SIGMA_MAX = 0.0
MAX_CONTROL_STEPS = COST_END_IDX - CONTROL_START_IDX
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp060_action_chunking"
        ckpt = None
        ...
        # Chunk state
        self._planned_deltas = []  # list of K floats (tanh-squashed * ds)
    ...
    def _build_obs(
        ...
        ctrl_step = max(0, (self.n - 1) - (CONTROL_START_IDX - CONTEXT_LENGTH))
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        ctrl_step = self.n - 1 - warmup_n  # 0-indexed control step
        if ctrl_step % CHUNK_K == 0 or not self._planned_deltas:
            obs = self._build_obs(
            ...
            with torch.no_grad():
                mu = self.ac.get_mu(torch.from_numpy(obs).unsqueeze(0))  # (1, K)

            # Deterministic: tanh(mu) * delta_scale for each of K steps
            self._planned_deltas = [
                math.tanh(mu[0, k].item()) * self.delta_scale for k in range(CHUNK_K)
            ]
            self._step_in_chunk = 0

        # Apply pre-planned delta
        delta = self._planned_deltas[self._step_in_chunk]
        action = float(np.clip(self._h_act[-1] + delta, STEER_RANGE[0], STEER_RANGE[1]))
