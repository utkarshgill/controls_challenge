...
def build_state(target_lataccel, current_lataccel, state, future_plan, flip=False):
    """Build normalized 55D state (no flip during eval)"""
...
class Controller(BaseController):
    """
    PPO with data augmentation (symmetric training)
    State: 55D normalized
    """
    def __init__(self, checkpoint_path=None):
        if checkpoint_path is None:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            checkpoint_path = os.path.join(current_dir, '../experiments/exp005_ppo_augmented/results/checkpoints/ppo_best.pth')
        ...
        print(f"✅ Loaded PPO (augmented) weights from {checkpoint_path}")
