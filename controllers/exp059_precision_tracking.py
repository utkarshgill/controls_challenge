...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        actor.append(nn.Linear(HIDDEN, 2))  # [mu_raw, log_sigma_raw]
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp059_precision_tracking"
        ckpt = None
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        # Deterministic: use tanh(mu) as squashed action
        raw = math.tanh(mu)
        delta = (
            raw * self.delta_scale
        )  # tanh already bounds to [-1,1], * ds gives [-ds, ds]
