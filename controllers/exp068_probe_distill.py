...
class ResidualChunkActor(nn.Module):
    ...
    def forward(self, obs, base_raw):
        ...
        return torch.tanh(self.net(x)) * self.max_residual
...
class Model(nn.Module):
    def __init__(self, chunk_k, max_residual):
        ...
        self.residual_actor = ResidualChunkActor(chunk_k, max_residual)
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp068_probe_distill"
        ckpt = None
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        with torch.no_grad():
            base_raw = float(self.model.base_raw(obs_t).item())
            if ((self.n - 1 - warmup_n) % self.chunk_k == 0) or not self._planned:
                resid = self.model.residual_chunk(obs_t).squeeze(0).cpu().numpy()
                self._planned = resid.tolist()
                self._step_in_chunk = 0

        residual = self._planned[self._step_in_chunk]
        self._step_in_chunk = min(self._step_in_chunk + 1, self.chunk_k - 1)
        raw = float(np.clip(base_raw + residual, -1.0, 1.0))
