...
def _criticality(target_series, current, chunk_k):
    horizon = min(len(target_series), max(2 * chunk_k, 8))
    ...
    mismatch_idx = min(chunk_k - 1, len(fp) - 1)
    mismatch = abs(float(fp[mismatch_idx]) - current) / S_LAT
...
class Controller(BaseController):
    def __init__(self):
        exp = Path(__file__).parent.parent / "experiments" / "exp070_iterated_probe_distill"
        ckpt = None
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        target_series = getattr(future_plan, "lataccel", None) or [target_lataccel]
        crit = _criticality(target_series, current_lataccel, self.chunk_k)
