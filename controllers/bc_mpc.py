"""BC controller trained on MPC expert data (exp110).

Same architecture as exp055 but loaded from bc_model.pt which contains
a plain state_dict (not the {"ac": ..., "delta_scale": ...} format).
"""
...
class Controller(BaseController):
    def __init__(self):
        ...
        if not Path(ckpt).exists():
            raise FileNotFoundError(f"BC model not found: {ckpt}")

        self.ac = ActorCritic()
        sd = torch.load(ckpt, weights_only=False, map_location="cpu")
        # bc_model.pt is a plain state_dict (actor.* + critic.*)
