...
class Controller(BaseController):
    """
    PPO with Tanh activations (symmetric)
    State: 55D normalized
    """
    def __init__(self, checkpoint_path=None):
        if checkpoint_path is None:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            checkpoint_path = os.path.join(current_dir, '../experiments/exp006_ppo_tanh/results/checkpoints/ppo_best.pth')
        ...
        print(f"✅ Loaded PPO (Tanh) weights from {checkpoint_path}")
